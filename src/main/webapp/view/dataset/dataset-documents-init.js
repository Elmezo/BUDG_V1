/**
 * Document Components Initialization for Dataset Edit Page
 * This script initializes the document upload and table components on page load
 * since they are now part of the DETAILS tab (not a separate tab)
 */
...
(function () {
    ...
    function initializeDocumentComponents() {
        ...
        if (!datasetId || isNaN(datasetId)) {
            console.error('Invalid dataset ID, cannot initialize document components');
            return;
        }

        console.log('Initializing document components for dataset ID:', datasetId);
    ...
    window.initDatasetDocuments = initializeDocumentComponents;
})();

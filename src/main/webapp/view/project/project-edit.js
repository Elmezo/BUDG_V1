...
function renderProjectTable(hierarchyRows, currentId) {
    const rowsHtml = hierarchyRows.rows.map(({ node, depth, childCount, hasChildren }) => {
        const name = node.primaryName ?? node.PrimaryName ?? node.Name ?? node.name ?? 'Unnamed Project';
        ...
        const link = `<a class="${linkClass}" href="/view/project/${encodeURIComponent(id)}" title="View ${escapeHtml(name)}">${escapeHtml(name)}</a>`;
        const refNumber = node.refNumber ? `<span class="project-ref">(${escapeHtml(node.refNumber)})</span>` : '';

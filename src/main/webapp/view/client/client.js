(function() {
    ...
    function renderClientTable(hierarchyRows, currentId) {
        ...
        const rowsHtml = hierarchyRows.rows.map(({ node, depth, childCount, hasChildren }) => {
            ...
            const fallbackName = node.primaryName ?? node.PrimaryName ?? node.Name ?? node.name ?? 'Unnamed Client';
            ...
            const link = isMaskedNode
                ...
                : `<a class="${linkClass}" href="/view/client/${encodeURIComponent(id)}" title="View ${escapeHtml(name)}">${escapeHtml(name)}</a>`;

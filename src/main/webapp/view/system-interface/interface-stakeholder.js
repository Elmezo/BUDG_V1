// interface Stakeholder View JavaScript
(function() {
    ...
    async function initStakeholderView(interfaceId) {
        currentinterfaceId = interfaceId;
        console.log('=== INITIALIZING STAKEHOLDER VIEW ===');
    ...
    async function loadStakeholders() {
        try {
            console.log('=== LOADING STAKEHOLDERS FOR VIEW ===');
            const container = document.getElementById('interfaceStakeholdersContainer');
            if (!container) {
                console.error('interface stakeholders container not found');
                return;
            }
            ...
            const apiUrl = `/api/interface-stakeholder/${currentinterfaceId}/stakeholders`;
            ...
            console.log('interface Stakeholders API Response:', responseData);
    ...
    // Get headers - show Role, Name, Department, Status, Delegate Of
    function getDynamicHeaders() {
        const t = (key, fallback) => (window.I18n && window.I18n.t(key)) || fallback;
        return [
            { key: 'role', display: t('systemInterface.stakeholder.columns.role', 'Role') },
            { key: 'name', display: t('systemInterface.stakeholder.columns.name', 'Name') },
            { key: 'department', display: t('systemInterface.stakeholder.columns.department', 'Department') },
            { key: 'status', display: t('systemInterface.stakeholder.columns.status', 'Status') },
            { key: 'delegateOf', display: t('systemInterface.stakeholder.columns.delegateOf', 'Delegate Of') }
        ];
    }
    ...
    function renderStakeholdersTable() {
        const container = document.getElementById('interfaceStakeholdersContainer');
        if (!container) {
            console.error('interface stakeholders container not found for rendering');
            return;
        }

        // Remove Person Id and Org Unit Id columns
        const headers = getDynamicHeaders().filter(h => h.key !== 'personId' && h.key !== 'orgUnitId');
        ...
        if (!stakeholdersData || stakeholdersData.length === 0) {
            html += `<tr><td colspan="${headers.length}"><div class='empty'>${(window.I18n && window.I18n.t('systemInterface.stakeholder.messages.noStakeholders')) || 'No stakeholders found'}</div></td></tr>`;
        } else {
            // For each stakeholder row
            stakeholdersData.forEach(row => {
                html += '<tr>';
                headers.forEach(h => {
                    ...
                    if (h.key === 'status') {
                        ...
                        const isCurrentUser = currentUserId && row.personId && currentUserId == row.personId;
                        const statusLower = statusValue.toLowerCase();
                        const isNoOrEmptyStatus = statusLower === 'no' || statusLower === '' || statusValue === '';
                        
                        if (isCurrentUser && isNoOrEmptyStatus && row.objectXPeopleId) {
                            // Make status editable with click handler - use objectXPeopleId for API call
                            const displayText = statusValue || ((window.I18n && window.I18n.t('systemInterface.stakeholder.messages.notAccepted')) || 'Not Accepted');
                            html += `<td><span class="status-badge ${statusClass} status-editable" onclick="InterfaceStakeholderView.toggleUserStatus(${row.objectXPeopleId})" style="cursor: pointer;" title="Click to accept">${escapeHtml(displayText)}</span></td>`;
                        } else {
                            // Read-only status
                            const displayText = statusValue || ((window.I18n && window.I18n.t('systemInterface.stakeholder.messages.notAccepted')) || 'Not Accepted');
                        ...
                        // Make department a hyperlink if orgUnitId exists
                        if (row.orgUnitId) {
                            html += `<td><a href="/view/orgunit/${row.orgUnitId}" target="_blank">${row[h.key]}</a></td>`;
                        } else {
                            html += `<td>${row[h.key] || ''}</td>`;
                        }
                    } else if (h.key === 'name' && row.personId) {
                        html += `<td><a href="/view/people/${encodeURIComponent(row.personId)}" class="stakeholder-link">${escapeHtml(value)}</a></td>`;
                    } else {
    ...
    async function toggleUserStatus(objectXPeopleId) {
        try {
            ...
            const response = await fetch(`/api/interface-stakeholder/${currentinterfaceId}/stakeholders/accept`, {
                method: 'PATCH',
                credentials: 'include',
                headers: {
                    'Content-Type': 'application/json'
    ...
    window.InterfaceStakeholderView = {

// Business Area Stakeholder View JavaScript
(function() {
    ...
    async function initStakeholderView(businessAreaId) {
    ...
    async function loadStakeholders() {
        try {
            ...
            const container = document.getElementById('businessAreaStakeholdersContainer');
            if (!container) {
                console.error('Business area stakeholders container not found');
                return;
            }
            ...
            const apiUrl = `/api/businessarea-stakeholder/${currentBusinessAreaId}/stakeholders`;
            ...
            console.log('Business Area Stakeholders API Response:', responseData);
    ...
    function getDynamicHeaders() {
        ...
        return [
            { key: 'role', display: t('businessArea.stakeholder.columns.role', 'Role') },
            { key: 'name', display: t('businessArea.stakeholder.columns.name', 'Name') },
            { key: 'department', display: t('businessArea.stakeholder.columns.department', 'Department') },
            { key: 'status', display: t('businessArea.stakeholder.columns.status', 'Status') },
            { key: 'delegateOf', display: t('businessArea.stakeholder.columns.delegateOf', 'Delegate Of') }
        ];
    }
    ...
    function renderStakeholdersTable() {
        const container = document.getElementById('businessAreaStakeholdersContainer');
        if (!container) {
            console.error('Business area stakeholders container not found for rendering');
            return;
        }
        ...
        if (stakeholdersData.length === 0) {
            ...
            html += `
                <tr>
                    <td colspan="${colspan}" class="no-data">${(window.I18n && window.I18n.t('businessArea.stakeholder.messages.noStakeholders')) || 'No stakeholders found'}</td>
                </tr>
            `;
        } else {
                ...
                headers.forEach(header => {
                    ...
                    if (header.key === 'status') {
                        ...
                        if (isCurrentUser && isNoOrEmptyStatus && stakeholder.objectXPeopleId) {
                            ...
                            const displayText = statusValue || ((window.I18n && window.I18n.t('businessArea.stakeholder.messages.notAccepted')) || 'Not Accepted');
                            html += `<td><span class="status-badge ${statusClass} status-editable" onclick="BusinessAreaStakeholderView.toggleUserStatus(${stakeholder.objectXPeopleId})" style="cursor: pointer;" title="Click to accept">${escapeHtml(displayText)}</span></td>`;
                        } else {
                            // Read-only status
                            const displayText = statusValue || ((window.I18n && window.I18n.t('businessArea.stakeholder.messages.notAccepted')) || 'Not Accepted');
        ...
        html += `
            ...
            <div id="businessAreaStakeholderCommunityContainer"></div>
        `;

        container.innerHTML = html;
        
        // Initialize stakeholder community section after main stakeholders table is rendered
        if (window.BusinessAreaStakeholderCommunity) {
            window.BusinessAreaStakeholderCommunity.init(currentBusinessAreaId);
        }
    }
    ...
    async function toggleUserStatus(objectXPeopleId) {
        try {
            ...
            const response = await fetch(`/api/businessarea-stakeholder/${currentBusinessAreaId}/stakeholders/accept`, {
                method: 'PATCH',
                credentials: 'include',
                headers: {
                    'Content-Type': 'application/json'
    ...
    window.BusinessAreaStakeholderView = {

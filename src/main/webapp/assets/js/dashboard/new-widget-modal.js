...
class NewWidgetModal {
    ...
    init() {
        ...
        if (okBtn) {
            ...
            newOkBtn.addEventListener('click', (e) => {
                ...
                console.log(`[NewWidget] Button disabled state:`, newOkBtn.disabled);
                console.log(`[NewWidget] Event target:`, e.target);
                
                if (!newOkBtn.disabled) {
                    this.handleSubmit();
                } else {
                    console.log(`[NewWidget] Button is disabled, ignoring click`);
                }
            });
        }
    ...
    showImageToolbar(imageElement) {
        ...
        if (content) {
            ...
            infoSpan.textContent = this._t('dashboard.modal.editingImage', 'Editing image…');
    ...
    showTableToolbar(cellElement) {
        ...
        if (content) {
            ...
            infoSpan.textContent = this._t('dashboard.modal.editingTable', 'Editing table…');
            infoSpan.style.fontSize = '0.8rem';
            infoSpan.style.color = '#6c757d';
            infoSpan.style.marginBottom = '4px';
            toolbar.insertBefore(infoSpan, toolbar.firstChild);
    ...
    resetForm() {
        ...
        if (this.modal) {
            ...
            if (modalTitle) {
                modalTitle.textContent = (window.I18n && window.I18n.t) ? window.I18n.t('dashboard.modal.newWidget') : 'New Widget';
            }
    ...
    async loadFacets(searchId = null) {
        try {
            // Build URL with searchId parameter if provided
            let url = '/api/dashboard/new-widget/facets';
            if (searchId) {
            ...
            console.log(`[NewWidget] Loaded ${this.facets.length} facets${searchId ? ' for search ' + searchId : ''}`);
    ...
    async handleSourceSelectChange(searchId) {
        ...
        // Load facets for the selected search (only facets with count > 0)
        if (searchId) {
            await this.loadFacets(searchId);
        } else {
            // If no search selected, load all facets
    ...
    async handleFocusChange(facetName) {
        ...
        // Update visualization options based on facet
        this.updateVisualizationOptions();
        
        // Populate fields based on current visualization type
        const currentVisualizeType = document.querySelector('input[name="visualizeAs"]:checked')?.value;
        if (currentVisualizeType === 'table') {
            // For table, populate display chips with ALL columns
            this.populateDisplayChips();
        } else if (currentVisualizeType === 'doughnut' || currentVisualizeType === 'bar') {
            // For charts, populate visualizeBy select with dropdown columns only
    ...
    renderPreview(previewData) {
        ...
        if (previewData.type === 'count') {
            // Always show the selected facet's count (not all facets)
            preview.innerHTML = `
                <div class="preview-count">
                    <div class="preview-count-number">${previewData.count || 0}</div>
                    <div class="preview-count-label">${previewData.facet || ''}</div>
                </div>
            `;
            ...
            // Do NOT fall back to allFacetCounts - show error if no data
            if (previewData.data && previewData.data.length > 0 && previewData.columns && previewData.columns.length > 0) {
                this.renderTablePreview(preview, previewData);
            } else {
                // No table data available - show error message
                preview.innerHTML = `
                    <div class="preview-error">
                        <p>${this.escapeHtml(this._t('dashboard.preview.noDataTable', 'No data available for table. Please check that the selected display columns have data for the filtered objects.'))}</p>
                    </div>
                `;
            }
    ...
    renderTablePreview(container, previewData) {
        ...
        if (!previewData.columns || !previewData.data) {
            container.innerHTML = '<div class="preview-error"><p>' + this.escapeHtml(this._t('dashboard.preview.noData', 'No data available')) + '</p></div>';
            return;
        }
        ...
        if (previewData.data.length === 0) {
            html += `<tr><td colspan="${previewData.columns.length}" style="text-align: center; padding: 2rem;">${this.escapeHtml(this._t('dashboard.preview.noDataFound', 'No data found'))}</td></tr>`;
        } else {
    ...
    renderChartPreview(container, previewData) {
        ...
        console.log('previewData.allFacetCounts:', previewData.allFacetCounts);
        console.log('previewData.data keys length:', previewData.data ? Object.keys(previewData.data).length : 'data is null/undefined');
        
        // ALWAYS use the selected facet's chart data (grouped by visualizeBy column)
        // Do NOT fall back to allFacetCounts - show error if no data
        const hasChartData = previewData.data && Object.keys(previewData.data).length > 0;
        
        let chartLabels = [];
        let chartValues = [];
        let chartTitle = previewData.column || 'Count';
        let facetName = previewData.facet || previewData.focus || '';
        
        if (hasChartData) {
            // Use the selected facet's chart data (e.g., 2 datasets grouped by status)
            console.log('Using selected facet chart data for visualization');
            chartLabels = Object.keys(previewData.data);
            chartValues = Object.values(previewData.data);
        } else {
            // No chart data available - show error message
            console.log('No chart data for selected facet - showing error message');
            container.innerHTML = '<div class="preview-error"><p>' + this.escapeHtml(this._t('dashboard.preview.noDataChart', 'No data available for chart. Please check that the selected column has data for the filtered objects.')) + '</p></div>';
            return;
        }
        
        if (chartLabels.length === 0) {
            console.log('No chart data after processing - showing error message');
            container.innerHTML = '<div class="preview-error"><p>' + this.escapeHtml(this._t('dashboard.preview.noDataChartShort', 'No data available for chart')) + '</p></div>';
            return;
        }
            ...
            const config = {
                ...
                data: {
                    ...
                    datasets: [{
                        label: chartTitle,
                        data: chartValues,
                        backgroundColor: this.generateColors(chartLabels.length),
                        borderColor: '#ffffff',
                        borderWidth: 1
                    }]
                },
                options: {
                    ...
                    plugins: {
                        legend: {
                            position: previewData.type === 'doughnut' ? 'right' : 'top',
                            display: true,
                            labels: {
                                padding: 10,
                                font: {
    ...
    renderAllFacetsCountPreview(container, previewData) {
        ...
        let html = `
            <div class="preview-all-facets-count">
                <div class="preview-count">
                    <div class="preview-count-number">${totalCount}</div>
                    <div class="preview-count-label">Total Results</div>
                </div>
                <div class="preview-facets-breakdown" style="margin-top: 1rem; max-height: 200px; overflow-y: auto;">
        `;
        ...
        facetsList.forEach(facet => {
            html += `
                <div style="display: flex; justify-content: space-between; padding: 0.25rem 0; border-bottom: 1px solid #eee;">
                    <span>${this.escapeHtml(facet.name)}</span>
                    <span style="font-weight: bold;">${facet.count}</span>
                </div>
            `;
        });
    ...
    renderAllFacetsTablePreview(container, previewData) {
        const allFacetCounts = previewData.allFacetCounts || {};
        ...
        let html = '<div class="preview-table-container" style="max-height: 300px; overflow-y: auto;">';
        html += '<table style="width: 100%; border-collapse: collapse;">';
        html += '<thead><tr style="background: #f5f5f5; border-bottom: 2px solid #ddd;">';
        html += '<th style="padding: 0.5rem; text-align: left;">Facet</th>';
        html += '<th style="padding: 0.5rem; text-align: right;">Count</th>';
        html += '</tr></thead><tbody>';

        if (facetsList.length === 0) {
            html += '<tr><td colspan="2" style="text-align: center; padding: 1rem;">' + this.escapeHtml(this._t('dashboard.preview.noDataFound', 'No data found')) + '</td></tr>';
        } else {
            facetsList.forEach(facet => {
                html += '<tr style="border-bottom: 1px solid #eee;">';
                html += `<td style="padding: 0.5rem;">${this.escapeHtml(facet.name)}</td>`;
                html += `<td style="padding: 0.5rem; text-align: right; font-weight: bold;">${facet.count}</td>`;
                html += '</tr>';
            });
        }
        ...
        html += `<div style="padding: 0.5rem; background: #f5f5f5; border-top: 2px solid #ddd; font-weight: bold;">Total: ${totalCount} results across ${facetsList.length} facet(s)</div>`;
        html += '</div>';
    ...
    async handleSubmit() {
        ...
        if (okBtn) {
            okBtn.disabled = true;
            okBtn.textContent = (window.I18n && window.I18n.t) ? (this.isEditMode() ? window.I18n.t('dashboard.updating') : window.I18n.t('dashboard.creating')) : (this.isEditMode() ? 'Updating...' : 'Creating...');
            ...
            if (response.ok && result.success) {
                // Get dashboard ID and widget ID BEFORE closing modal (to preserve them)
                ...
                if (window.dashboard) {
                    ...
                    // If editing, get the widget ID being edited
                    if (this.isEditMode()) {
                        editingWidgetId = this.modal.getAttribute('data-widget-id');
                        console.log(`[NewWidget] EDIT MODE DETECTED - Editing widget ID: ${editingWidgetId}`);
                        console.log(`[NewWidget] Modal edit mode attribute:`, this.modal.getAttribute('data-edit-mode'));
                        console.log(`[NewWidget] Modal widget ID attribute:`, this.modal.getAttribute('data-widget-id'));
                    } else {
                        console.log(`[NewWidget] CREATE MODE DETECTED`);
                    }
                }
                ...
                // Handle edit vs create differently
                if (editingWidgetId) {
                    // EDIT MODE: Refresh the existing widget
                    console.log(`[NewWidget] EDIT MODE CONFIRMED - refreshing existing widget ${editingWidgetId}`);
                    
                    if (window.dashboard) {
                        try {
                            // Fetch updated widget data
                            const widgetResponse = await fetch(`/api/dashboard/widgets/${editingWidgetId}`, {
                                credentials: 'include'
                            });
                            
                            if (widgetResponse.ok) {
                                const updatedWidgetData = await widgetResponse.json();
                                console.log(`[NewWidget] Updated widget data:`, updatedWidgetData);
                                
                                // Find and update the existing widget
                                const containerId = `widget${editingWidgetId}Widget`;
                                const existingContainer = document.getElementById(containerId);
                                
                                if (existingContainer && updatedWidgetData) {
                                    console.log(`[NewWidget] FOUND EXISTING CONTAINER: ${containerId}`);
                                    console.log(`[NewWidget] Updated widget data:`, updatedWidgetData);
                                    console.log(`[NewWidget] Widget type:`, updatedWidgetData.type);
                                    
                                    // Clear the existing container first
                                    existingContainer.innerHTML = '';
                                    console.log(`[NewWidget] Cleared existing container content`);
                                    
                                    // Re-initialize the widget with updated data
                                    if (updatedWidgetData.type === 'text') {
                                        console.log(`[NewWidget] Re-initializing TextWidget`);
                                        const widget = new TextWidget(containerId, updatedWidgetData);
                                        await widget.init();
                                        window.dashboard.widgets[`widget${editingWidgetId}`] = widget;
                                        console.log(`[NewWidget] TextWidget re-initialized successfully`);
                                    } else if (updatedWidgetData.type === 'saved_search') {
                                        console.log(`[NewWidget] Re-initializing SavedSearchWidget`);
                                        const widget = new SavedSearchWidget(containerId, updatedWidgetData);
                                        await widget.init();
                                        window.dashboard.widgets[`widget${editingWidgetId}`] = widget;
                                        console.log(`[NewWidget] SavedSearchWidget re-initialized successfully`);
                                    } else {
                                        console.error(`[NewWidget] Unknown widget type: ${updatedWidgetData.type}`);
                                    }
                                    
                                    // Re-setup controls for the updated widget
                                    setTimeout(() => {
                                        try {
                                            if (window.dashboard) {
                                                console.log(`[NewWidget] Setting up controls for updated widget`);
                                                const widgetElement = existingContainer.querySelector('.dashboard-widget');
                                                if (widgetElement) {
                                                    console.log(`[NewWidget] Found widget element, adding controls`);
                                                    window.dashboard.addDragHandle(widgetElement);
                                                    window.dashboard.addWidgetMenu(widgetElement);
                                                    console.log(`[NewWidget] Controls added successfully`);
                                                } else {
                                                    console.error(`[NewWidget] Widget element not found in container`);
                                                    console.error(`[NewWidget] Container content:`, existingContainer.innerHTML);
                                                }
                                            }
                                        } catch (controlError) {
                                            console.error(`[NewWidget] Error setting up controls:`, controlError);
                                        }
                                    }, 100);
                                    
                                    console.log(`[NewWidget] Widget ${editingWidgetId} updated successfully`);
                                    
                                    // Re-enable the OK button for next use
                                    const okBtn = document.getElementById('newWidgetOK');
                                    if (okBtn) {
                                        okBtn.disabled = false;
                                        okBtn.textContent = (window.I18n && window.I18n.t) ? window.I18n.t('button.ok') : 'OK';
                                        console.log(`[NewWidget] OK button re-enabled after edit`);
                                    }
                                } else {
                                    console.error(`[NewWidget] CONTAINER NOT FOUND for widget ${editingWidgetId}`);
                                    console.error(`[NewWidget] Expected container ID: ${containerId}`);
                                    console.error(`[NewWidget] Available containers:`, Array.from(document.querySelectorAll('[id$="Widget"]')).map(el => el.id));
                                    console.error(`[NewWidget] Widget data received:`, updatedWidgetData);
                                    // Fallback to full reload
                                    await window.dashboard.loadDashboardWidgets(dashboardId);
                                }
                            } else {
                                console.error(`[NewWidget] Failed to fetch updated widget data`);
                                // Fallback to full reload
                                await window.dashboard.loadDashboardWidgets(dashboardId);
                            }
                        } catch (error) {
                            console.error(`[NewWidget] Error updating widget:`, error);
                            // Fallback to full reload
                            await window.dashboard.loadDashboardWidgets(dashboardId);
                        }
                    }
                } else {
                    // CREATE MODE: Add new widget (existing logic)
                    if (window.dashboard) {
                            ...
                            if (widgetResponse.ok) {
                                ...
                                if (widgetData && widgetData.id) {
                                    ...
                                    setTimeout(() => {
                                        try {
                                            if (window.dashboard) {
                                                console.log(`[NewWidget] Setting up controls for new widget`);
                                                
                                                // Setup controls for all widgets (including the new one)
                                                window.dashboard.setupWidgetControls();
                                                
                                                // Setup drag and drop for all widgets
                                                if (window.dashboard.dragDrop) {
                                                    console.log(`[NewWidget] Setting up drag and drop`);
                                            ...
                                            console.error(`[NewWidget] Error during widget setup:`, setupError);
                                            console.error(`[NewWidget] Setup error stack:`, setupError.stack);
                                        }
                                    }, 200);
                                    ...
                                    if (okBtn) {
                                        okBtn.disabled = false;
                                        okBtn.textContent = (window.I18n && window.I18n.t) ? window.I18n.t('button.ok') : 'OK';
                                        console.log(`[NewWidget] OK button re-enabled for next use`);
                                    }
                ...
                if (okBtn) {
                    okBtn.disabled = false;
                    okBtn.textContent = originalText;
                    console.log(`[NewWidget] OK button re-enabled after inner error`);
                }
            }
        } catch (error) {
            console.error(`[NewWidget] OUTER ERROR:`, error);
            const okBtn = document.getElementById('newWidgetOK');
            if (okBtn) {
                okBtn.disabled = false;
                okBtn.textContent = (window.I18n && window.I18n.t) ? window.I18n.t('button.ok') : 'OK';
     ...
     */
    isEditMode() {
        const editMode = this.modal && this.modal.getAttribute('data-edit-mode') === 'true';
        console.log(`[NewWidget] isEditMode() called - result: ${editMode}`);
        console.log(`[NewWidget] Modal exists: ${!!this.modal}`);
        console.log(`[NewWidget] data-edit-mode attribute: ${this.modal ? this.modal.getAttribute('data-edit-mode') : 'modal not found'}`);
        return editMode;
    }
}

...
/**
 * Client Stakeholder Servlet for handling stakeholder CRUD operations
 * Endpoints: /api/client/{id}/stakeholders, /api/client/stakeholder/lookup
 */
@WebServlet({"/api/client-stakeholder/*", "/api/client/stakeholder/lookup"})
public class ClientStakeholderServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(ClientStakeholderServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            //system.out.println("ClientStakeholderServlet - Request URI: " + requestURI);
            //system.out.println("ClientStakeholderServlet - Path Info: " + pathInfo);
            ...
            if (pathInfo != null && (pathInfo.matches("/\\d+/stakeholders(/edit)?") || pathInfo.equals("/1/stakeholders") || pathInfo.equals("/1/stakeholders/edit"))) {
                ...
                int clientId = extractClientId(requestURI);
                //system.out.println("Extracted client ID: " + clientId);

                if (clientId > 0) {
                    List<JsonObject> stakeholders = getClientStakeholders(conn, clientId);
                    //system.out.println("Found " + stakeholders.size() + " stakeholders");
                    out.print(gson.toJson(stakeholders));
                } else {
                    //system.out.println("Could not extract client ID from URI: " + requestURI);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                int clientId = extractClientId(request.getRequestURI());
                ...
                // Check Edit permission + stakeholder status for Client
                if (!PermissionCheckUtil.checkEditPermissionWithStakeholder(request, response, "Client", clientId)) {
                ...
                DefaultStakeholderUtil.ValidationResult canSave = DefaultStakeholderUtil.validateCurrentUserCanSaveStakeholders(
                        conn, "Client", clientId, currentUserId, java.util.Collections.emptySet());
                if (!canSave.isValid()) {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                ...
                if (jsonObject.has("ipid")) {
                    ...
                    SegmentValidationService.ValidationResult segmentValidation =
                        segmentValidationService.validateStakeholderCanBeAdded(personId, clientId, "Client");
                ...
                // Add stakeholder
                boolean success = addStakeholder(conn, clientId, jsonObject, currentUserId);
    ...
    protected void doPatch(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/accept")) {
                int clientId = extractClientId(requestURI);
                //system.out.println("PATCH - Extracted client ID: " + clientId);
    ...
    // Get client stakeholders (works for both VIEW and EDIT modes)
    private List<JsonObject> getClientStakeholders(Connection conn, int clientId) throws SQLException {
        String sql = """
            SELECT 
    ...
    private List<JsonObject> getClientRoles(Connection conn) throws SQLException {
        String sql = """
            SELECT 
                orl.id AS RoleID,
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, getModuleId(conn, "Client"));
    ...
    private List<JsonObject> getPeopleByRoleWithSegmentFilter(Connection conn, int roleId, int objectId) throws SQLException {
        ...
        try {
            List<Map<String, Object>> users = SegmentAccessService.getUsersForStakeholderSelection(objectId, "Client", roleId);
    ...
    // Add stakeholder
    private boolean addStakeholder(Connection conn, int clientId, JsonObject data, int currentUserId) throws SQLException {
        ...
        try {
            ...
            // Insert into client_x_objectxpeople
            String insertCXOP = """
            ...
            try (PreparedStatement stmt = conn.prepareStatement(insertCXOP)) {
                ...
                int rowsAffected = stmt.executeUpdate();
                if (rowsAffected == 0) {
                    throw new SQLException("Failed to insert client_x_objectxpeople");
                }
            }
            ...
            insertStakeholderAudit(conn, clientId, "Name", null, fullName, "link", "Added", userName);
            insertStakeholderAudit(conn, clientId, "Role", null, roleName, "link", "Added", userName);
            insertStakeholderAudit(conn, clientId, "Role Status", null, statusName, "link", "Added", userName);
            ...
            com.example.budg_v2.util.RoleNotificationHelper.createNotificationAfterStakeholderAdded(
                "Client", clientId, data.get("ipid").getAsInt(), 
    ...
    private boolean updateStakeholder(Connection conn, JsonObject data, int currentUserId) throws SQLException {
        ...
        int clientId = getClientIdForStakeholder(conn, objectXPeopleId);
        String userName = "System";
        ...
        if (changed && oldData != null && newData != null) {
            if (!safeEquals(oldData.fullName, newData.fullName))
                insertStakeholderAudit(conn, clientId, "Name", oldData.fullName, newData.fullName, "edit", "Updated", userName);
            if (!safeEquals(oldData.roleName, newData.roleName))
                insertStakeholderAudit(conn, clientId, "Role", oldData.roleName, newData.roleName, "edit", "Updated", userName);
            if (!safeEquals(oldData.statusName, newData.statusName))
                insertStakeholderAudit(conn, clientId, "Role Status", oldData.statusName, newData.statusName, "edit", "Updated", userName);
        }
        return true;
    }
    ...
    private boolean deleteStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        ...
        Integer facetTypeId = facetChangesDAO.getFacetId("client");
        if (facetTypeId != null && facetChangesDAO.hasActiveCRsBlockingStakeholderDeletion(facetTypeId, clientId)) {
        ...
        try {
            // Delete from client_x_objectxpeople first (foreign key constraint)
            String deleteCXOP = "DELETE FROM client_x_objectxpeople WHERE Object_X_ipid = ?";
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, clientId, "Name", oldData.fullName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, clientId, "Role", oldData.roleName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, clientId, "Role Status", oldData.statusName, null, "delete", "Deleted", userName);
            }
    ...
    // Extract client ID from request URI
    private int extractClientId(String requestURI) {
        String[] uriParts = requestURI.split("/");
        for (int i = 0; i < uriParts.length; i++) {
            if ("client-stakeholder".equals(uriParts[i]) && i + 1 < uriParts.length) {
                try {
    ...
    private int getClientIdForStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        String sql = "SELECT ClientID FROM client_x_objectxpeople WHERE Object_X_ipid = ? ORDER BY id DESC LIMIT 1";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, objectXPeopleId);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) return rs.getInt("ClientID");
            }
        }
        return -1;
    }
    ...
    /**
     * Build reference string in the format "FacetName ObjectId" (e.g., "Client 5")
     */
    ...
    private void insertStakeholderAudit(Connection conn, int clientId, String field, String oldValue, String newValue, String event, String updateType, String userName) throws SQLException {
        String mappedField = mapStakeholderField(field);
        String sql = "INSERT INTO client_audit_history (id, object, event, updateType, field, `from`, `to`, author) VALUES (?, 'Stakeholders', ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, clientId);
    ...
    private boolean acceptStakeholderStatus(Connection conn, int objectXPeopleId, int currentUserId) throws SQLException {
        ...
        // Get client ID and old data for audit trail
        int clientId = getClientIdForStakeholder(conn, objectXPeopleId);
        StakeholderSnapshot oldData = getStakeholderInfo(conn, objectXPeopleId);
        String userName = "System";
        try { userName = getCurrentUserNameAttribute(conn); } catch (Exception ignore) {}
        ...
        try {
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, clientId, "Role Status", oldData.statusName, "Yes", "status", "Accepted", userName);
            }

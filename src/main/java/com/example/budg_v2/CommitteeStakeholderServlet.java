...
/**
 * Committee Stakeholder Servlet for handling stakeholder CRUD operations
 * Endpoints: /api/committee/{id}/stakeholders, /api/committee/stakeholder/lookup
 */
@WebServlet({"/api/committee-stakeholder/*", "/api/committee/stakeholder/lookup"})
public class CommitteeStakeholderServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(CommitteeStakeholderServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            //system.out.println("CommitteeStakeholderServlet - Request URI: " + requestURI);
            //system.out.println("CommitteeStakeholderServlet - Path Info: " + pathInfo);
            ...
            if (pathInfo != null && (pathInfo.matches("/\\d+/stakeholders(/edit)?") || pathInfo.equals("/1/stakeholders") || pathInfo.equals("/1/stakeholders/edit"))) {
                ...
                int committeeId = extractCommitteeId(requestURI);
                //system.out.println("Extracted committee ID: " + committeeId);

                if (committeeId > 0) {
                    List<JsonObject> stakeholders = getCommitteeStakeholders(conn, committeeId);
                    //system.out.println("Found " + stakeholders.size() + " stakeholders");
                    out.print(gson.toJson(stakeholders));
                } else {
                    //system.out.println("Could not extract committee ID from URI: " + requestURI);
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    out.print("{\"error\": \"Invalid committee ID\"}");
                }
            } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                int committeeId = extractCommitteeId(request.getRequestURI());
                ...
                // Check Edit permission + stakeholder status for Committee
                ...
                DefaultStakeholderUtil.ValidationResult canSave = DefaultStakeholderUtil.validateCurrentUserCanSaveStakeholders(
                        conn, "Committee", committeeId, currentUserId, java.util.Collections.emptySet());
                if (!canSave.isValid()) {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                ...
                if (jsonObject.has("ipid")) {
                    ...
                    SegmentValidationService.ValidationResult segmentValidation =
                        segmentValidationService.validateStakeholderCanBeAdded(personId, committeeId, "Committee");
                ...
                // Add stakeholder
                boolean success = addStakeholder(conn, committeeId, jsonObject, currentUserId, request);
    ...
    protected void doPatch(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/accept")) {
                int committeeId = extractCommitteeId(requestURI);
                //system.out.println("PATCH - Extracted committee ID: " + committeeId);
    ...
    private void handleLookupRequest(HttpServletRequest request, HttpServletResponse response,
        ...
        switch (type.toLowerCase()) {
            case "roles":
                results = getCommitteeRoles(conn);
                break;
            case "rolestatus":
    ...
    // Get committee stakeholders (works for both VIEW and EDIT modes)
    private List<JsonObject> getCommitteeStakeholders(Connection conn, int committeeId) throws SQLException {
        String sql = """
            SELECT 
    ...
    // Get committee roles
    private List<JsonObject> getCommitteeRoles(Connection conn) throws SQLException {
        String sql = """
            SELECT 
                orl.id AS RoleID,
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, getModuleId(conn, "Committee"));
    ...
    private List<JsonObject> getPeopleByRoleWithSegmentFilter(Connection conn, int roleId, int objectId) throws SQLException {
        ...
        try {
            List<Map<String, Object>> users = SegmentAccessService.getUsersForStakeholderSelection(objectId, "Committee", roleId);
    ...
    // Add stakeholder
    private boolean addStakeholder(Connection conn, int committeeId, JsonObject data, int currentUserId, HttpServletRequest request) throws SQLException {
        ...
        try {
            ...
            // Insert into committee_x_objectxpeople
            String insertCXOP = """
            ...
            try (PreparedStatement stmt = conn.prepareStatement(insertCXOP)) {
                ...
                int rowsAffected = stmt.executeUpdate();
                if (rowsAffected == 0) {
                    throw new SQLException("Failed to insert committee_x_objectxpeople");
                }
            }
            ...
            insertStakeholderAudit(conn, committeeId, "Name", null, fullName, "link", "Added", userName);
            insertStakeholderAudit(conn, committeeId, "Role", null, roleName, "link", "Added", userName);
            insertStakeholderAudit(conn, committeeId, "Role Status", null, statusName, "link", "Added", userName);
            ...
            com.example.budg_v2.util.RoleNotificationHelper.createNotificationAfterStakeholderAdded(
                "Committee", committeeId, data.get("ipid").getAsInt(), 
    ...
    private boolean updateStakeholder(Connection conn, JsonObject data, int currentUserId) throws SQLException {
        ...
        int committeeId = getCommitteeIdForStakeholder(conn, objectXPeopleId);
        ...
        if (changed && oldData != null && newData != null) {
            if (!safeEquals(oldData.fullName, newData.fullName))
                insertStakeholderAudit(conn, committeeId, "Name", oldData.fullName, newData.fullName, "edit", "Updated", userName);
            if (!safeEquals(oldData.roleName, newData.roleName))
                insertStakeholderAudit(conn, committeeId, "Role", oldData.roleName, newData.roleName, "edit", "Updated", userName);
            if (!safeEquals(oldData.statusName, newData.statusName))
                insertStakeholderAudit(conn, committeeId, "Role Status", oldData.statusName, newData.statusName, "edit", "Updated", userName);
        }
        return true;
    }
    ...
    private boolean deleteStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        ...
        Integer facetTypeId = facetChangesDAO.getFacetId("committee");
        if (facetTypeId != null && facetChangesDAO.hasActiveCRsBlockingStakeholderDeletion(facetTypeId, committeeId)) {
        ...
        try {
            // Delete from committee_x_objectxpeople first (foreign key constraint)
            String deleteCXOP = "DELETE FROM committee_x_objectxpeople WHERE Object_X_ipid = ?";
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, committeeId, "Name", oldData.fullName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, committeeId, "Role", oldData.roleName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, committeeId, "Role Status", oldData.statusName, null, "delete", "Deleted", userName);
            }
    ...
    // Extract committee ID from request URI
    private int extractCommitteeId(String requestURI) {
        String[] uriParts = requestURI.split("/");
        for (int i = 0; i < uriParts.length; i++) {
            if ("committee-stakeholder".equals(uriParts[i]) && i + 1 < uriParts.length) {
                try {
    ...
    private int getCommitteeIdForStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        String sql = "SELECT Committee_ID FROM committee_x_objectxpeople WHERE Object_X_ipid = ? ORDER BY id DESC LIMIT 1";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, objectXPeopleId);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) return rs.getInt("Committee_ID");
            }
        }
        return -1;
    }
    ...
    /**
     * Build reference string in the format "FacetName ObjectId" (e.g., "Committee 5")
     */
    ...
    private void insertStakeholderAudit(Connection conn, int committeeId, String field, String oldValue, String newValue, String event, String updateType, String userName) throws SQLException {
        String mappedField = mapStakeholderField(field);
        String sql = "INSERT INTO committee_audit_history (id, object, event, updateType, field, `from`, `to`, author) VALUES (?, 'Stakeholders', ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, committeeId);
    ...
    private boolean acceptStakeholderStatus(Connection conn, int objectXPeopleId, int currentUserId) throws SQLException {
        ...
        try {
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, committeeId, "Role Status", oldData.statusName, "Yes", "status", "Accepted", userName);
            }

...
@WebServlet(name = "CapabilityDashboardServlet", urlPatterns = {"/api/capability/dashboard/stats"})
public class CapabilityDashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try (Connection conn = com.example.budg_v2.database.DatabaseConnection.getConnection()) {
            ...
            // Build WHERE clause for filtering capability records
            String filterWhereClause = buildFilterWhereClause(filters, conn);
            
            // Build segment filter condition
            String segmentFilter = buildSegmentFilter(userId, "capability.id");
            
            // First, get total count of filtered non-deleted capability records for verification
            String totalCountSql = "SELECT COUNT(*) as total FROM capability WHERE DeletedDatetime IS NULL" + filterWhereClause + segmentFilter;
            ...
            List<Map<String, Object>> statusStats = getStatusStats(conn, filterWhereClause, filters, segmentFilter);
            result.put("status", statusStats);
            
            List<Map<String, Object>> classificationStats = getClassificationStats(conn, filterWhereClause, filters, segmentFilter);
    ...
    /**
     * Build WHERE clause to exclude filtered capability records
     */
    private String buildFilterWhereClause(Map<String, Set<String>> filters, Connection conn) throws SQLException {
        ...
        // Exclude by Lifecycle (no nulls for lifecycle)
        if (filters.containsKey("lifecycle") && !filters.get("lifecycle").isEmpty()) {
            ...
            if (!lifecycleIds.isEmpty()) {
                ...
                conditions.add("capability.Lifecycle NOT IN (" + idsStr + ")");
            }
        }
        
        // Exclude by Status (no nulls for status)
        if (filters.containsKey("status") && !filters.get("status").isEmpty()) {
            ...
            if (!statusIds.isEmpty()) {
                ...
                conditions.add("capability.Status NOT IN (" + idsStr + ")");
            }
        }
        ...
        if (filters.containsKey("classification") && !filters.get("classification").isEmpty()) {
            ...
            if (!nonNullNames.isEmpty()) {
                ...
                if (!classificationIds.isEmpty()) {
                    ...
                    conditions.add("capability.Classification NOT IN (" + idsStr + ")");
                }
            }
            
            // Exclude null values if "null" is in the filter
            if (excludeNull) {
                conditions.add("capability.Classification IS NOT NULL");
            }
        }
        ...
        if (filters.containsKey("type") && !filters.get("type").isEmpty()) {
            ...
            if (!typeIds.isEmpty()) {
                ...
                conditions.add("capability.Capability_Type NOT IN (" + idsStr + ")");
            }
        }
    ...
    private List<Integer> getLifecycleIdsByName(Connection conn, Set<String> names) throws SQLException {
        ...
        String sql = "SELECT ID FROM capability_lifecyle WHERE PrimaryName IN (" + placeholders + ")";
    ...
    private List<Integer> getClassificationIdsByName(Connection conn, Set<String> names) throws SQLException {
        ...
        String placeholders = validNames.stream().map(n -> "?").collect(java.util.stream.Collectors.joining(","));
        String sql = "SELECT ID FROM capability_classification WHERE PrimaryName IN (" + placeholders + ")";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            int idx = 1;
            for (String name : validNames) {
                ps.setString(idx++, name.trim());
    ...
    private List<Integer> getTypeIdsByName(Connection conn, Set<String> names) throws SQLException {
        ...
        String placeholders = names.stream().map(n -> "?").collect(java.util.stream.Collectors.joining(","));
        String sql = "SELECT ID FROM capability_type WHERE PrimaryName IN (" + placeholders + ")";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            int idx = 1;
            for (String name : names) {
                ps.setString(idx++, name.trim());
    ...
    /**
     * Build segment filter condition for capability
     */
    private String buildSegmentFilter(Integer userId, String idColumn) {
        if (userId != null && userId > 0) {
            try {
                String segmentCondition = SegmentAccessService.buildSelectedSegmentFilterClause(userId, "Capability", idColumn);
     ...
     * Note: Lifecycle does NOT include nulls
     */
    private List<Map<String, Object>> getLifecycleStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT cl.PrimaryName as label, COALESCE(COUNT(c.ID), 0) as count " +
                     "FROM capability_lifecyle cl " +
                     "LEFT JOIN (SELECT * FROM capability WHERE DeletedDatetime IS NULL" + filterWhereClause + segmentFilter + ") c ON c.Lifecycle = cl.ID " +
                     "GROUP BY cl.ID, cl.PrimaryName " +
                     "ORDER BY cl.ID";
    ...
    /**
     * Build WHERE clause excluding a specific field for null count queries
     */
    private String buildNullFilterClause(Map<String, Set<String>> filters, String excludeField, Connection conn) throws SQLException {
        if (filters.isEmpty()) {
            return "";
        }
        
        Map<String, Set<String>> filteredFilters = new HashMap<>(filters);
        filteredFilters.remove(excludeField);
        
        if (filteredFilters.isEmpty()) {
            return "";
        }
        
        return buildFilterWhereClause(filteredFilters, conn);
    }
    
    /**
     * Get Status stats - returns ALL status options with counts (including 0)
     */
    private List<Map<String, Object>> getStatusStats(Connection conn, String filterWhereClause, Map<String, Set<String>> filters, String segmentFilter) throws SQLException {
        List<Map<String, Object>> stats = new ArrayList<>();
        
        String sql = "SELECT COALESCE(s.PrimaryName, s.primaryname) as label, COALESCE(COUNT(c.ID), 0) as count " +
                     "FROM status s " +
                     "LEFT JOIN (SELECT * FROM capability WHERE DeletedDatetime IS NULL" + filterWhereClause + segmentFilter + ") c ON c.Status = s.id " +
     ...
     */
    private List<Map<String, Object>> getClassificationStats(Connection conn, String filterWhereClause, Map<String, Set<String>> filters, String segmentFilter) throws SQLException {
        List<Map<String, Object>> stats = new ArrayList<>();
        
        String sql = "SELECT cc.PrimaryName as label, COALESCE(COUNT(c.ID), 0) as count " +
                     "FROM capability_classification cc " +
                     "LEFT JOIN (SELECT * FROM capability WHERE DeletedDatetime IS NULL" + filterWhereClause + segmentFilter + ") c ON c.Classification = cc.ID " +
                     "GROUP BY cc.ID, cc.PrimaryName " +
                     "ORDER BY cc.ID";
        ...
        // Build filter clause without Classification filter for null count
        String nullFilterClause = buildNullFilterClause(filters, "classification", conn);
        String nullSql = "SELECT COUNT(*) as count FROM capability WHERE Classification IS NULL AND DeletedDatetime IS NULL" + nullFilterClause + segmentFilter;
        try (PreparedStatement ps = conn.prepareStatement(nullSql);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                int nullCount = rs.getInt("count");
    ...
    private List<Map<String, Object>> getTypeStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT ct.PrimaryName as label, COALESCE(COUNT(c.ID), 0) as count " +
                     "FROM capability_type ct " +
                     "LEFT JOIN (SELECT * FROM capability WHERE DeletedDatetime IS NULL" + filterWhereClause + segmentFilter + ") c ON c.Capability_Type = ct.ID " +
                     "GROUP BY ct.ID, ct.PrimaryName " +
                     "ORDER BY ct.ID";

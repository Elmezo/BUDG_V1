...
/**
 * capability Stakeholder Servlet for handling stakeholder CRUD operations
 * Endpoints: /api/capability/{id}/stakeholders, /api/capability/stakeholder/lookup
 */
@WebServlet({"/api/capability-stakeholder/*", "/api/capability/stakeholder/lookup"})
public class CapabilityStakeholderServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(CapabilityStakeholderServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            //system.out.println("capabilityStakeholderServlet - Request URI: " + requestURI);
            //system.out.println("capabilityStakeholderServlet - Path Info: " + pathInfo);
            ...
            if (pathInfo != null && (pathInfo.matches("/\\d+/stakeholders(/edit)?") || pathInfo.equals("/1/stakeholders") || pathInfo.equals("/1/stakeholders/edit"))) {
                ...
                int capabilityId = extractcapabilityId(requestURI);
                //system.out.println("Extracted capability ID: " + capabilityId);

                if (capabilityId > 0) {
                    List<JsonObject> stakeholders = getcapabilityStakeholders(conn, capabilityId);
                    //system.out.println("Found " + stakeholders.size() + " stakeholders");
                    out.print(gson.toJson(stakeholders));
                } else {
                    //system.out.println("Could not extract capability ID from URI: " + requestURI);
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    out.print("{\"error\": \"Invalid capability ID\"}");
                }
            } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                int capabilityId = extractcapabilityId(request.getRequestURI());
                ...
                // Check Edit permission + stakeholder status for Capability
                ...
                DefaultStakeholderUtil.ValidationResult canSave = DefaultStakeholderUtil.validateCurrentUserCanSaveStakeholders(
                        conn, "Capability", capabilityId, currentUserId, java.util.Collections.emptySet());
                if (!canSave.isValid()) {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                ...
                if (jsonObject.has("ipid")) {
                    ...
                    SegmentValidationService.ValidationResult segmentValidation =
                        segmentValidationService.validateStakeholderCanBeAdded(personId, capabilityId, "Capability");
                ...
                // Add stakeholder
                boolean success = addStakeholder(conn, capabilityId, jsonObject, currentUserId);
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                ...
                if (jsonObject.has("ipid")) {
                    ...
                    SegmentValidationService.ValidationResult segmentValidation =
                        segValSvc.validateStakeholderCanBeAdded(personId, capabilityId, "Capability");
                    if (!segmentValidation.isValid) {
                        response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                        out.print("{\"error\": " + gson.toJson(segmentValidation.message) + "}");
                        return;
    ...
    protected void doPatch(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/accept")) {
                int capabilityId = extractcapabilityId(requestURI);
                //system.out.println("PATCH - Extracted capability ID: " + capabilityId);
    ...
    private void handleLookupRequest(HttpServletRequest request, HttpServletResponse response,
        ...
        switch (type.toLowerCase()) {
            case "roles":
                results = getcapabilityRoles(conn);
                break;
            case "rolestatus":
    ...
    // Get capability stakeholders (works for both VIEW and EDIT modes)
    private List<JsonObject> getcapabilityStakeholders(Connection conn, int capabilityId) throws SQLException {
        String sql = """
            SELECT 
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    stakeholder.add("AcceptedID", acceptedID == 0 ? JsonNull.INSTANCE : new com.google.gson.JsonPrimitive(acceptedID));
    ...
    private List<JsonObject> getcapabilityRoles(Connection conn) throws SQLException {
        String sql = """
            SELECT 
                orl.id AS RoleID,
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, getModuleId(conn, "capability"));
    ...
    private List<JsonObject> getPeopleByRoleWithSegmentFilter(Connection conn, int roleId, int objectId) throws SQLException {
        ...
        try {
            List<Map<String, Object>> users = SegmentAccessService.getUsersForStakeholderSelection(objectId, "Capability", roleId);
    ...
    // Add stakeholder
    private boolean addStakeholder(Connection conn, int capabilityId, JsonObject data, int currentUserId) throws SQLException {
        ...
        try {
            ...
            // Insert into capability_x_objectxpeople
            String insertCXOP = """
            ...
            try (PreparedStatement stmt = conn.prepareStatement(insertCXOP)) {
                ...
                int rowsAffected = stmt.executeUpdate();
                if (rowsAffected == 0) {
                    throw new SQLException("Failed to insert capability_x_objectxpeople");
                }
            }
            ...
            insertStakeholderAudit(conn, capabilityId, "Name", null, fullName, "link", "Added", userName);
            insertStakeholderAudit(conn, capabilityId, "Role", null, roleName, "link", "Added", userName);
            insertStakeholderAudit(conn, capabilityId, "Role Status", null, statusName, "link", "Added", userName);
            ...
            com.example.budg_v2.util.RoleNotificationHelper.createNotificationAfterStakeholderAdded(
                "Capability", capabilityId, data.get("ipid").getAsInt(), 
    ...
    private boolean updateStakeholder(Connection conn, JsonObject data, int currentUserId) throws SQLException {
        ...
        int capabilityId = getCapabilityIdForStakeholder(conn, objectXPeopleId);
        String userName = "System";
        ...
        if (changed && oldData != null && newData != null) {
            if (!safeEquals(oldData.fullName, newData.fullName))
                insertStakeholderAudit(conn, capabilityId, "Name", oldData.fullName, newData.fullName, "edit", "Updated", userName);
            if (!safeEquals(oldData.roleName, newData.roleName))
                insertStakeholderAudit(conn, capabilityId, "Role", oldData.roleName, newData.roleName, "edit", "Updated", userName);
            if (!safeEquals(oldData.statusName, newData.statusName))
                insertStakeholderAudit(conn, capabilityId, "Role Status", oldData.statusName, newData.statusName, "edit", "Updated", userName);
        }
        return true;
    }
    ...
    private boolean deleteStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        ...
        Integer facetTypeId = facetChangesDAO.getFacetId("capability");
        if (facetTypeId != null && facetChangesDAO.hasActiveCRsBlockingStakeholderDeletion(facetTypeId, capabilityId)) {
        ...
        try {
            // Delete from capability_x_objectxpeople first (foreign key constraint)
            String deleteCXOP = "DELETE FROM capability_x_objectxpeople WHERE Object_x_ipid = ?";
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, capabilityId, "Name", oldData.fullName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, capabilityId, "Role", oldData.roleName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, capabilityId, "Role Status", oldData.statusName, null, "delete", "Deleted", userName);
            }
    ...
    // Extract capability ID from request URI
    private int extractcapabilityId(String requestURI) {
        String[] uriParts = requestURI.split("/");
        for (int i = 0; i < uriParts.length; i++) {
            if ("capability-stakeholder".equals(uriParts[i]) && i + 1 < uriParts.length) {
                try {
    ...
    private int getCapabilityIdForStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        String sql = "SELECT CapabilityID FROM capability_x_objectxpeople WHERE Object_x_ipid = ? ORDER BY id DESC LIMIT 1";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, objectXPeopleId);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) return rs.getInt("CapabilityID");
            }
        }
        return -1;
    }
    ...
    /**
     * Build reference string in the format "FacetName ObjectId" (e.g., "Capability 3")
     */
    ...
    private void insertStakeholderAudit(Connection conn, int capabilityId, String field, String oldValue, String newValue, String event, String updateType, String userName) throws SQLException {
        String mappedField = mapStakeholderField(field);
        String sql = "INSERT INTO capability_audit_history (id, object, event, updateType, field, `from`, `to`, author) VALUES (?, 'Stakeholders', ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, capabilityId);
    ...
    private boolean acceptStakeholderStatus(Connection conn, int objectXPeopleId, int currentUserId) throws SQLException {
        ...
        try {
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, capabilityId, "Role Status", oldData.statusName, "Yes", "status", "Accepted", userName);
            }

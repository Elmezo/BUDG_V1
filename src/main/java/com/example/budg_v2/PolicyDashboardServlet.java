...
@WebServlet(name = "PolicyDashboardServlet", urlPatterns = {"/api/policy/dashboard/stats"})
public class PolicyDashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try (Connection conn = com.example.budg_v2.database.DatabaseConnection.getConnection()) {
            ...
            // Build WHERE clause for filtering policy records
            String filterWhereClause = buildFilterWhereClause(filters, conn);
            
            // Build segment filter condition
            String segmentFilter = buildSegmentFilter(userId, "policy.ID");
            
            // First, get total count of filtered non-deleted policy records for verification
            String totalCountSql = "SELECT COUNT(*) as total FROM policy WHERE DeletedDatetime IS NULL" + filterWhereClause + segmentFilter;
    ...
    /**
     * Build WHERE clause to exclude filtered policy records
     */
    private String buildFilterWhereClause(Map<String, Set<String>> filters, Connection conn) throws SQLException {
        ...
        if (filters.containsKey("type") && !filters.get("type").isEmpty()) {
            ...
            if (!typeIds.isEmpty()) {
                ...
                conditions.add("policy.Policy_Type NOT IN (" + idsStr + ")");
            }
        }
        ...
        if (filters.containsKey("status") && !filters.get("status").isEmpty()) {
            ...
            if (!statusIds.isEmpty()) {
                ...
                conditions.add("policy.status NOT IN (" + idsStr + ")");
            }
        }
        ...
        if (filters.containsKey("lifecycle") && !filters.get("lifecycle").isEmpty()) {
            ...
            if (!lifecycleIds.isEmpty()) {
                ...
                conditions.add("policy.Lifecycle_Status NOT IN (" + idsStr + ")");
            }
        }
    ...
    private List<Integer> getTypeIdsByName(Connection conn, Set<String> names) throws SQLException {
        ...
        String sql = "SELECT id FROM policy_type WHERE PrimaryName IN (" + placeholders + ")";
    ...
    private List<Integer> getLifecycleIdsByName(Connection conn, Set<String> names) throws SQLException {
        ...
        String placeholders = validNames.stream().map(n -> "?").collect(java.util.stream.Collectors.joining(","));
        String sql = "SELECT id FROM policy_lifecycle_status WHERE PrimaryName IN (" + placeholders + ")";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            int idx = 1;
            for (String name : validNames) {
                ps.setString(idx++, name.trim());
    ...
    /**
     * Build segment filter condition for policy
     */
    private String buildSegmentFilter(Integer userId, String idColumn) {
        if (userId != null && userId > 0) {
            try {
                String segmentCondition = SegmentAccessService.buildSelectedSegmentFilterClause(userId, "Policy", idColumn);
    ...
    private List<Map<String, Object>> getTypeStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT pt.PrimaryName as label, COALESCE(COUNT(po.ID), 0) as count " +
                     "FROM policy_type pt " +
                     "LEFT JOIN (SELECT * FROM policy WHERE DeletedDatetime IS NULL" + filterWhereClause + segmentFilter + ") po ON po.Policy_Type = pt.id " +
    ...
    private List<Map<String, Object>> getStatusStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT COALESCE(s.PrimaryName, s.primaryname) as label, COALESCE(COUNT(po.ID), 0) as count " +
                     "FROM status s " +
                     "LEFT JOIN (SELECT * FROM policy WHERE DeletedDatetime IS NULL" + filterWhereClause + segmentFilter + ") po ON po.status = s.id " +
    ...
    private List<Map<String, Object>> getLifecycleStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT pls.PrimaryName as label, COALESCE(COUNT(po.ID), 0) as count " +
                     "FROM policy_lifecycle_status pls " +
                     "LEFT JOIN (SELECT * FROM policy WHERE DeletedDatetime IS NULL" + filterWhereClause + segmentFilter + ") po ON po.Lifecycle_Status = pls.id " +

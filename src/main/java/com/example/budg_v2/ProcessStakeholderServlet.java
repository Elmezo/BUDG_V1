...
/**
 * Process Stakeholder Servlet for handling stakeholder CRUD operations
 * Endpoints: /api/process/{id}/stakeholders, /api/process/stakeholder/lookup
 */
@WebServlet({"/api/process-stakeholder/*", "/api/process/stakeholder/lookup"})
public class ProcessStakeholderServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(ProcessStakeholderServlet.class);
    ...
    @SuppressWarnings("unused") // reserved for process context
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            //////system.out.println("ProcessStakeholderServlet - Request URI: " + requestURI);
            //////system.out.println("ProcessStakeholderServlet - Path Info: " + pathInfo);
            ...
            // Handle stakeholders view requests
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/view")) {
                int originalProcessId = extractProcessId(requestURI);
                ...
                if (originalProcessId > 0) {
                    ...
                    List<JsonObject> stakeholders = getProcessStakeholdersForView(conn, originalProcessId);
                    out.print(gson.toJson(stakeholders));
                } else {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    out.print("{\"error\": \"Invalid process ID\"}");
                }
            }
            // Handle stakeholders edit requests  
            else if (pathInfo != null && (pathInfo.matches("/\\d+/stakeholders/edit") || pathInfo.equals("/1/stakeholders/edit"))) {
                int originalProcessId = extractProcessId(requestURI);

                if (originalProcessId > 0) {
                    // Stakeholders are excluded from pending changes - always load from original
                    // Changes to stakeholders are applied immediately even with active CR
                    List<JsonObject> stakeholders = getProcessStakeholdersForEdit(conn, originalProcessId);
                    out.print(gson.toJson(stakeholders));
                } else {
                    //////system.out.println("Could not extract process ID from URI: " + requestURI);
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    out.print("{\"error\": \"Invalid process ID\"}");
                }
            } else {
                //////system.out.println("No matching pattern for pathInfo: " + pathInfo);
                //////system.out.println("Expected pattern: /\\d+/stakeholders/edit");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                int processId = extractProcessId(request.getRequestURI());
                ...
                // Check Edit permission + stakeholder status for Process
                ...
                DefaultStakeholderUtil.ValidationResult canSave = DefaultStakeholderUtil.validateCurrentUserCanSaveStakeholders(
                        conn, "Process", processId, currentUserId, java.util.Collections.emptySet());
                if (!canSave.isValid()) {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                ...
                if (jsonObject.has("ipid")) {
                    ...
                    SegmentValidationService.ValidationResult segmentValidation =
                        segmentValidationService.validateStakeholderCanBeAdded(personId, processId, "Process");
                ...
                // Add stakeholder directly to original process (no pending changes for stakeholders)
                boolean success = addStakeholder(conn, processId, jsonObject, currentUserId, request);
    ...
    protected void doPut(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                ...
                // Check Edit permission for Process
                PermissionService permissionService = new PermissionService();
                if (!permissionService.canEdit(currentUserId, "Process")) {
                    response.setStatus(HttpServletResponse.SC_FORBIDDEN);
                    out.print("{\"error\": \"You don't have permission to edit Process\"}");
                    return;
                }
                ...
                // If process is under active CR, ensure stakeholders are initialized in cloned process
                ...
                try {
                    ...
                    if (activeCrId == null) {
                        try {
                            ...
                            Integer autoCrId = dfcrService.applyDefaultsOnEdit("Process", processId, processType, currentUserId, isAdmin);
                            if (autoCrId != null) {
                                logger.info("[ProcessStakeholderServlet] DFCR auto-created CR: {}", autoCrId);
                                activeCrId = autoCrId;
                            }
                        } catch (Exception e) {
                            logger.error("[ProcessStakeholderServlet] Error checking/creating DFCR CR: {}", e.getMessage());
                        }
                    }
                    ...
                    if (activeCrId != null) {
                        ...
                        if (nobjectId != null) {
                            Integer stakeholdersMapping = facetChangesDAO.getNObjectId("process", processId, "stakeholders#process_stakeholder", activeCrId);
                            if (stakeholdersMapping == null) {
                                initializeStakeholdersInClonedProcess(conn, processId, nobjectId, currentUserId);
                                facetChangesDAO.saveMapping("process", processId, nobjectId, "stakeholders#process_stakeholder", activeCrId);
                            }
                            logger.info("Updating stakeholder in cloned process ID {} (original: {})", nobjectId, processId);
                        }
                    }
                } catch (SQLException e) {
                    logger.warn("Error checking active CR for process stakeholders PUT: {}", e.getMessage());
                }
                ...
                // Update stakeholder (updateStakeholder uses objectXPeopleId which is shared, so it should work)
                boolean success = updateStakeholder(conn, jsonObject, currentUserId);
                //////system.out.println("Update stakeholder result: " + success);

                if (success) {
                    if (activeCrId != null) {
                        out.print("{\"success\": true, \"pending\": true, \"changeRequestId\": " + activeCrId +
                                ", \"message\": \"Stakeholder change saved as pending. It will apply when the Change Request is completed.\"}");
                    } else {
    ...
    protected void doDelete(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            //////system.out.println("DELETE objectXPeopleId: " + objectXPeopleId);
            ...
            //////system.out.println("Parsed oxpId: " + oxpId);

            // Delete stakeholder
            boolean success = deleteStakeholder(conn, oxpId);
            //////system.out.println("Delete stakeholder result: " + success);
    ...
    protected void doPatch(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/accept")) {
                int processId = extractProcessId(requestURI);
                //system.out.println("PATCH - Extracted process ID: " + processId);
    ...
    // Get process stakeholders for VIEW mode (uses AcceptedID from roleaccepted)
    private List<JsonObject> getProcessStakeholdersForView(Connection conn, int processId) throws SQLException {
        String sql = """
            SELECT 
    ...
    // Get process stakeholders for EDIT mode (uses statusID from object_x_ip_status)
    private List<JsonObject> getProcessStakeholdersForEdit(Connection conn, int processId) throws SQLException {
        String sql = """
            SELECT 
                oxp.id AS ObjectXPeopleID,
                oxp.isDelegateOF AS DelegateOfId,
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    stakeholder.addProperty("PersonEmail", rs.getString("PersonEmail"));
                    stakeholder.addProperty("OrgUnit", rs.getString("OrgUnit"));
                    stakeholder.addProperty("OrgUnitID", rs.getInt("OrgUnitID"));
                    // Handle DelegateOfId - can be NULL
                    int delegateOfId = rs.getInt("DelegateOfId");
                    if (rs.wasNull()) {
                        stakeholder.add("DelegateOfId", null);
    ...
    // Get process roles
    private List<JsonObject> getProcessRoles(Connection conn) throws SQLException {
        String sql = """
            SELECT 
                orl.id AS RoleID,
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, getModuleId(conn, "Process"));
    ...
    private List<JsonObject> getPeopleByRoleWithSegmentFilter(Connection conn, int roleId, int objectId) throws SQLException {
        ...
        try {
            List<Map<String, Object>> users = SegmentAccessService.getUsersForStakeholderSelection(objectId, "Process", roleId);
    ...
    // Add stakeholder
    private boolean addStakeholder(Connection conn, int processId, JsonObject data, int currentUserId, HttpServletRequest request) throws SQLException {
        //////system.out.println("Adding stakeholder for processId: " + processId);
        ...
        try {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(insertOXP, Statement.RETURN_GENERATED_KEYS)) {
                ipid = data.get("ipid").getAsInt();
                roleID = data.get("roleID").getAsInt();
            ...
            // Insert into process_x_objectxpeople
            String insertPXOP = """
            ...
            try (PreparedStatement stmt = conn.prepareStatement(insertPXOP)) {
                ...
                //system.out.println("🔍 Inserting into process_x_objectxpeople with:");
                //system.out.println("   - objectXPeopleId: " + objectXPeopleId);
                //system.out.println("   - processId: " + processId);
                //system.out.println("   - currentUserId: " + currentUserId);

                int rowsAffected = stmt.executeUpdate();
                //system.out.println("✅ process_x_objectxpeople insert result: " + rowsAffected + " rows affected");
            ...
            insertStakeholderAudit(conn, processId, "Name", null, fullName, "link", "Added", userName);
            insertStakeholderAudit(conn, processId, "Role", null, roleName, "link", "Added", userName);
            insertStakeholderAudit(conn, processId, "Role Status", null, statusName, "link", "Added", userName);
            ...
            com.example.budg_v2.util.RoleNotificationHelper.createNotificationAfterStakeholderAdded(
                "Process", processId, data.get("ipid").getAsInt(), 
    ...
    /**
     * Initialize stakeholders in a cloned process by copying all stakeholders from the original process.
     */
    private void initializeStakeholdersInClonedProcess(Connection conn, int originalProcessId, int clonedProcessId, int currentUserId) throws SQLException {
        // Copy process_x_objectxpeople link rows (object_x_people rows are shared).
        String sql = """
            INSERT INTO process_x_objectxpeople (object_x_ip, process_id, lastupdate_userid)
            SELECT pxop.object_x_ip, ?, ?
            FROM process_x_objectxpeople pxop
            WHERE pxop.process_id = ?
            """;
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            stmt.setInt(3, originalProcessId);
            stmt.executeUpdate();
        }
    }

    /**
     * Clone a process row for pending changes (same columns as ProcessServlet.cloneProcessRow), using existing connection.
     */
    ...
    private boolean updateStakeholder(Connection conn, JsonObject data, int currentUserId) throws SQLException {
        ...
        int processId = getProcessIdForStakeholder(conn, objectXPeopleId);
        ...
        if (changed && oldData != null && newData != null) {
            if (!safeEquals(oldData.fullName, newData.fullName))
                insertStakeholderAudit(conn, processId, "Name", oldData.fullName, newData.fullName, "edit", "Updated", userName);
            if (!safeEquals(oldData.roleName, newData.roleName))
                insertStakeholderAudit(conn, processId, "Role", oldData.roleName, newData.roleName, "edit", "Updated", userName);
            if (!safeEquals(oldData.statusName, newData.statusName))
                insertStakeholderAudit(conn, processId, "Role Status", oldData.statusName, newData.statusName, "edit", "Updated", userName);
        }
        return true;
    }
    ...
    private boolean deleteStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        ...
        Integer facetTypeId = facetChangesDAO.getFacetId("process");
        if (facetTypeId != null && facetChangesDAO.hasActiveCRsBlockingStakeholderDeletion(facetTypeId, processId)) {
        ...
        try {
            // Delete from process_x_objectxpeople first (foreign key constraint)
            String deletePXOP = "DELETE FROM process_x_objectxpeople WHERE object_x_ip = ?";
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, processId, "Name", oldData.fullName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, processId, "Role", oldData.roleName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, processId, "Role Status", oldData.statusName, null, "delete", "Deleted", userName);
            }
    ...
    private boolean acceptStakeholderStatus(Connection conn, int objectXPeopleId, int currentUserId) throws SQLException {
        ...
        // Get process ID and old data for audit trail
        int processId = getProcessIdForStakeholder(conn, objectXPeopleId);
        StakeholderSnapshot oldData = getStakeholderInfo(conn, objectXPeopleId);
        String userName = "System";
        try { userName = getCurrentUserNameAttribute(conn); } catch (Exception ignore) {}
        ...
        try {
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, processId, "Role Status", oldData.statusName, "Yes", "status", "Accepted", userName);
            }
    ...
    // Extract process ID from request URI
    private int extractProcessId(String requestURI) {
        String[] uriParts = requestURI.split("/");
        for (int i = 0; i < uriParts.length; i++) {
            if ("process-stakeholder".equals(uriParts[i]) && i + 1 < uriParts.length) {
                try {
    ...
    private int getProcessIdForStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        String sql = "SELECT process_id FROM process_x_objectxpeople WHERE object_x_ip = ? ORDER BY id DESC LIMIT 1";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, objectXPeopleId);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) return rs.getInt("process_id");
            }
        }
        return -1;
    }
    ...
    /**
     * Build reference string in the format "FacetName ObjectId" (e.g., "Process 7")
     */
    ...
    private void insertStakeholderAudit(Connection conn, int processId, String field, String oldValue, String newValue, String event, String updateType, String userName) throws SQLException {
        String mappedField = mapStakeholderField(field);
        String sql = "INSERT INTO process_audit_history (id, object, event, updateType, field, `from`, `to`, author) VALUES (?, 'Stakeholders', ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, processId);

...
@WebServlet(name = "CommitteeDashboardServlet", urlPatterns = {"/api/committee/dashboard/stats"})
public class CommitteeDashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try (Connection conn = com.example.budg_v2.database.DatabaseConnection.getConnection()) {
            ...
            // Build WHERE clause for filtering committee records
            String filterWhereClause = buildFilterWhereClause(filters, conn);
            
            // Build segment filter condition
            String segmentFilter = buildSegmentFilter(userId, "committee.id");
            
            // First, get total count of filtered non-deleted committee records for verification
            String totalCountSql = "SELECT COUNT(*) as total FROM committee WHERE DeleteDatetime IS NULL" + filterWhereClause + segmentFilter;
    ...
    /**
     * Build WHERE clause to exclude filtered committee records
     */
    private String buildFilterWhereClause(Map<String, Set<String>> filters, Connection conn) throws SQLException {
        ...
        if (filters.containsKey("type") && !filters.get("type").isEmpty()) {
            ...
            if (!typeIds.isEmpty()) {
                ...
                conditions.add("committee.Committee_Type NOT IN (" + idsStr + ")");
            }
        }
        ...
        if (filters.containsKey("lifecycle") && !filters.get("lifecycle").isEmpty()) {
            ...
            if (!lifecycleIds.isEmpty()) {
                ...
                conditions.add("committee.Lifecycle NOT IN (" + idsStr + ")");
            }
        }
        ...
        if (filters.containsKey("classification") && !filters.get("classification").isEmpty()) {
            ...
            if (!classificationIds.isEmpty()) {
                ...
                conditions.add("committee.Classification NOT IN (" + idsStr + ")");
            }
        }
        ...
        if (filters.containsKey("status") && !filters.get("status").isEmpty()) {
            ...
            if (!statusIds.isEmpty()) {
                ...
                conditions.add("committee.Status NOT IN (" + idsStr + ")");
            }
        }
    ...
    private List<Integer> getTypeIdsByName(Connection conn, Set<String> names) throws SQLException {
        ...
        String sql = "SELECT ID FROM committee_type WHERE PrimaryName IN (" + placeholders + ")";
    ...
    private List<Integer> getLifecycleIdsByName(Connection conn, Set<String> names) throws SQLException {
        ...
        String placeholders = validNames.stream().map(n -> "?").collect(java.util.stream.Collectors.joining(","));
        String sql = "SELECT ID FROM committee_lifecycle WHERE PrimaryName IN (" + placeholders + ")";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            int idx = 1;
            for (String name : validNames) {
                ps.setString(idx++, name.trim());
    ...
    private List<Integer> getClassificationIdsByName(Connection conn, Set<String> names) throws SQLException {
        ...
        String placeholders = validNames.stream().map(n -> "?").collect(java.util.stream.Collectors.joining(","));
        String sql = "SELECT ID FROM committee_classification WHERE PrimaryName IN (" + placeholders + ")";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            int idx = 1;
            for (String name : validNames) {
                ps.setString(idx++, name.trim());
    ...
    /**
     * Build segment filter condition for committee
     */
    private String buildSegmentFilter(Integer userId, String idColumn) {
        if (userId != null && userId > 0) {
            try {
                String segmentCondition = SegmentAccessService.buildSelectedSegmentFilterClause(userId, "Committee", idColumn);
    ...
    private List<Map<String, Object>> getTypeStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT ct.PrimaryName as label, COALESCE(COUNT(c.ID), 0) as count " +
                     "FROM committee_type ct " +
                     "LEFT JOIN (SELECT * FROM committee WHERE DeleteDatetime IS NULL" + filterWhereClause + segmentFilter + ") c ON c.Committee_Type = ct.ID " +
    ...
    private List<Map<String, Object>> getLifecycleStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT cl.PrimaryName as label, COALESCE(COUNT(c.ID), 0) as count " +
                     "FROM committee_lifecycle cl " +
                     "LEFT JOIN (SELECT * FROM committee WHERE DeleteDatetime IS NULL" + filterWhereClause + segmentFilter + ") c ON c.Lifecycle = cl.ID " +
    ...
    private List<Map<String, Object>> getClassificationStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT cc.PrimaryName as label, COALESCE(COUNT(c.ID), 0) as count " +
                     "FROM committee_classification cc " +
                     "LEFT JOIN (SELECT * FROM committee WHERE DeleteDatetime IS NULL" + filterWhereClause + segmentFilter + ") c ON c.Classification = cc.ID " +
        ...
        String nullSql = "SELECT COUNT(*) as count FROM committee WHERE Classification IS NULL AND DeleteDatetime IS NULL" + filterWhereClause + segmentFilter;
    ...
    private List<Map<String, Object>> getStatusStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT COALESCE(s.PrimaryName, s.primaryname) as label, COALESCE(COUNT(c.ID), 0) as count " +
                     "FROM status s " +
                     "LEFT JOIN (SELECT * FROM committee WHERE DeleteDatetime IS NULL" + filterWhereClause + segmentFilter + ") c ON c.Status = s.id " +

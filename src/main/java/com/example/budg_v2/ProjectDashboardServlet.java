...
@WebServlet(name = "ProjectDashboardServlet", urlPatterns = {"/api/project/dashboard/stats"})
public class ProjectDashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try (Connection conn = com.example.budg_v2.database.DatabaseConnection.getConnection()) {
            ...
            // Build WHERE clause for filtering project records
            String filterWhereClause = buildFilterWhereClause(filters, conn);
            
            // Build segment filter condition
            String segmentFilter = buildSegmentFilter(userId, "project.id");
            
            // First, get total count of filtered non-deleted project records for verification
            String totalCountSql = "SELECT COUNT(*) as total FROM project WHERE deletedatetime IS NULL" + filterWhereClause + segmentFilter;
    ...
    /**
     * Build WHERE clause to exclude filtered project records
     */
    private String buildFilterWhereClause(Map<String, Set<String>> filters, Connection conn) throws SQLException {
        ...
        // Exclude by Lifecycle (note: column is lifecycle_status in project table)
        if (filters.containsKey("lifecycle") && !filters.get("lifecycle").isEmpty()) {
            ...
            if (!lifecycleIds.isEmpty()) {
                ...
                conditions.add("project.lifecycle_status NOT IN (" + idsStr + ")");
            }
        }
        ...
        if (filters.containsKey("status") && !filters.get("status").isEmpty()) {
            ...
            if (!statusIds.isEmpty()) {
                ...
                conditions.add("project.status NOT IN (" + idsStr + ")");
            }
        }
    ...
    private List<Integer> getLifecycleIdsByName(Connection conn, Set<String> names) throws SQLException {
        ...
        String sql = "SELECT id FROM project_lifecycle WHERE PrimaryName IN (" + placeholders + ")";
    ...
    /**
     * Build segment filter condition for project
     */
    private String buildSegmentFilter(Integer userId, String idColumn) {
        if (userId != null && userId > 0) {
            try {
                String segmentCondition = SegmentAccessService.buildSelectedSegmentFilterClause(userId, "Project", idColumn);
     ...
     * Note: project table uses lifecycle_status column
     */
    private List<Map<String, Object>> getLifecycleStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT pl.PrimaryName as label, COALESCE(COUNT(prj.id), 0) as count " +
                     "FROM project_lifecycle pl " +
                     "LEFT JOIN (SELECT * FROM project WHERE deletedatetime IS NULL" + filterWhereClause + segmentFilter + ") prj ON prj.lifecycle_status = pl.id " +
    ...
    private List<Map<String, Object>> getStatusStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT COALESCE(s.PrimaryName, s.primaryname) as label, COALESCE(COUNT(prj.id), 0) as count " +
                     "FROM status s " +
                     "LEFT JOIN (SELECT * FROM project WHERE deletedatetime IS NULL" + filterWhereClause + segmentFilter + ") prj ON prj.status = s.id " +

...
/**
 * System Stakeholder Servlet for handling stakeholder CRUD operations
 * Endpoints: /api/system/{id}/stakeholders, /api/system/stakeholder/lookup
 */
@WebServlet({"/api/system-stakeholder/*", "/api/system/stakeholder/lookup"})
public class SystemStakeholderServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(SystemStakeholderServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && (pathInfo.matches("/\\d+/stakeholders(/edit)?") || pathInfo.equals("/1/stakeholders") || pathInfo.equals("/1/stakeholders/edit"))) {
                ...
                int originalSystemId = extractSystemId(requestURI);
                //system.out.println("Extracted system ID: " + originalSystemId);

                if (originalSystemId > 0) {
                    ...
                    List<JsonObject> stakeholders = getSystemStakeholders(conn, originalSystemId);
                    //system.out.println("Found " + stakeholders.size() + " stakeholders");
                    out.print(gson.toJson(stakeholders));
                } else {
                    //system.out.println("Could not extract system ID from URI: " + requestURI);
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    out.print("{\"error\": \"Invalid system ID\"}");
                }
            } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            String pathInfo = request.getPathInfo();
            //system.out.println("POST request - Path Info: " + pathInfo);

            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                int systemId = extractSystemId(request.getRequestURI());
                //system.out.println("POST - Extracted system ID: " + systemId);
                ...
                //system.out.println("POST - Parsed JSON: " + jsonObject.toString());

                // Validate required fields
                if (!jsonObject.has("ipid") || !jsonObject.has("roleID") || !jsonObject.has("statusID")) {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    out.print("{\"error\": \"Missing required fields: ipid, roleID, and statusID are required\"}");
                    return;
                }
                ...
                // Check Edit permission + stakeholder status for System
                ...
                DefaultStakeholderUtil.ValidationResult canSave = DefaultStakeholderUtil.validateCurrentUserCanSaveStakeholders(
                        conn, "System", systemId, currentUserId, java.util.Collections.emptySet());
                if (!canSave.isValid()) {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                ...
                if (jsonObject.has("ipid")) {
                    ...
                    SegmentValidationService.ValidationResult segmentValidation =
                        segmentValidationService.validateStakeholderCanBeAdded(personId, systemId, "System");
                ...
                // Add stakeholder directly to original system (no pending changes for stakeholders)
                boolean success = addStakeholder(conn, systemId, jsonObject, currentUserId);
                ...
                //system.out.println("POST - Invalid endpoint: " + pathInfo);
                response.setStatus(HttpServletResponse.SC_NOT_FOUND);
                out.print("{\"error\": \"Invalid endpoint\"}");
            }
        } catch (SQLException e) {
            String msg = e.getMessage() != null ? e.getMessage() : "";
            System.err.println("POST - Database error: " + msg);
            e.printStackTrace();
            ...
            System.err.println("POST - Server error: " + e.getMessage());
            e.printStackTrace();
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            out.print("{\"error\": \"Server error: " + e.getMessage() + "\"}");
        }
    ...
    protected void doPut(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            String pathInfo = request.getPathInfo();
            //system.out.println("PUT request - Path Info: " + pathInfo);

            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                int systemId = extractSystemId(request.getRequestURI());
                //system.out.println("PUT - Extracted system ID: " + systemId);
                ...
                JsonObject jsonObject = JsonParser.parseString(requestBody).getAsJsonObject();
                //system.out.println("PUT - Parsed JSON: " + jsonObject.toString());

                // Validate required fields
                if (!jsonObject.has("objectXPeopleId") || !jsonObject.has("ipid") ||
                        !jsonObject.has("roleID") || !jsonObject.has("statusID")) {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    out.print("{\"error\": \"Missing required fields: objectXPeopleId, ipid, roleID, and statusID are required\"}");
                    return;
                }
                ...
                // Check Edit permission for System
                PermissionService permissionService = new PermissionService();
                if (!permissionService.canEdit(currentUserId, "System")) {
                    response.setStatus(HttpServletResponse.SC_FORBIDDEN);
                    out.print("{\"error\": \"You don't have permission to edit System\"}");
                    return;
                }
                ...
                if (jsonObject.has("ipid")) {
                    ...
                    SegmentValidationService.ValidationResult segmentValidation =
                        segValSvc.validateStakeholderCanBeAdded(personId, systemId, "System");
                    if (!segmentValidation.isValid) {
                        response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                        out.print("{\"error\": " + gson.toJson(segmentValidation.message) + "}");
                        return;
                ...
                if (success) {
                    //system.out.println("PUT - Stakeholder updated successfully");
                    out.print("{\"success\": true, \"message\": \"Stakeholder updated successfully\"}");
                } else {
                    //system.out.println("PUT - Failed to update stakeholder");
                    response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                    out.print("{\"error\": \"Failed to update stakeholder\"}");
                }
            } else {
                //system.out.println("PUT - Invalid endpoint: " + pathInfo);
                response.setStatus(HttpServletResponse.SC_NOT_FOUND);
                out.print("{\"error\": \"Invalid endpoint\"}");
            }
        } catch (SQLException e) {
            System.err.println("PUT - Database error: " + e.getMessage());
            e.printStackTrace();
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            out.print("{\"error\": \"Database error: " + e.getMessage() + "\"}");
        } catch (Exception e) {
            System.err.println("PUT - Server error: " + e.getMessage());
            e.printStackTrace();
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            out.print("{\"error\": \"Server error: " + e.getMessage() + "\"}");
        }
    ...
    protected void doDelete(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            //system.out.println("DELETE request - objectXPeopleId: " + objectXPeopleId);
            ...
            int oxpId;
            try {
                oxpId = Integer.parseInt(objectXPeopleId);
            } catch (NumberFormatException e) {
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                out.print("{\"error\": \"Invalid ObjectXPeople ID format\"}");
                return;
            }

            //system.out.println("DELETE - Attempting to delete stakeholder with ID: " + oxpId);

            // Delete stakeholder
            boolean success = deleteStakeholder(conn, oxpId);

            if (success) {
                //system.out.println("DELETE - Stakeholder deleted successfully");
                out.print("{\"success\": true, \"message\": \"Stakeholder deleted successfully\"}");
            } else {
                //system.out.println("DELETE - Failed to delete stakeholder");
                response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            ...
            if (e.getMessage() != null && e.getMessage().contains("Cannot delete stakeholder")) {
                ...
                System.err.println("DELETE - Database error: " + e.getMessage());
                e.printStackTrace();
                response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                out.print("{\"error\": \"Database error: " + e.getMessage() + "\"}");
            }
        } catch (Exception e) {
            System.err.println("DELETE - Server error: " + e.getMessage());
            e.printStackTrace();
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            out.print("{\"error\": \"Server error: " + e.getMessage() + "\"}");
        }
    ...
    protected void doPatch(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/accept")) {
                int systemId = extractSystemId(requestURI);
                //system.out.println("PATCH - Extracted system ID: " + systemId);
    ...
    // Get system stakeholders (works for both VIEW and EDIT modes)
    private List<JsonObject> getSystemStakeholders(Connection conn, int systemId) throws SQLException {
        String sql = """
            SELECT 
    ...
    // Get system roles
    private List<JsonObject> getSystemRoles(Connection conn) throws SQLException {
        String sql = """
            SELECT 
                orl.id AS RoleID,
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, getModuleId(conn, "System"));
    ...
    private List<JsonObject> getPeopleByRoleWithSegmentFilter(Connection conn, int roleId, int objectId) throws SQLException {
        ...
        try {
            List<Map<String, Object>> users = SegmentAccessService.getUsersForStakeholderSelection(objectId, "System", roleId);
    ...
    // Add stakeholder
    private boolean addStakeholder(Connection conn, int systemId, JsonObject data, int currentUserId) throws SQLException {
        //system.out.println("Adding stakeholder - SystemID: " + systemId + ", Data: " + data.toString());

        int ipid = data.get("ipid").getAsInt();
        int roleID = data.get("roleID").getAsInt();

        // Prevent duplicate: same person + same role already linked to this system
        String checkExists = """
            SELECT 1 FROM system_x_objectxpeople sxop
            JOIN object_x_people oxp ON oxp.id = sxop.Object_x_ipid
            WHERE sxop.SystemID = ? AND oxp.ipid = ? AND oxp.roleID = ?
            """;
        try (PreparedStatement checkStmt = conn.prepareStatement(checkExists)) {
            checkStmt.setInt(1, systemId);
            checkStmt.setInt(2, ipid);
            checkStmt.setInt(3, roleID);
            try (ResultSet rs = checkStmt.executeQuery()) {
                if (rs.next()) {
                    throw new SQLException("This person is already assigned to this role for this system.");
                }
            }
        }
        ...
        try {
            ...
            //system.out.println("Validated data - ipid: " + ipid + ", roleID: " + roleID);

            // Read statusID from request; default to 1 (Active) if missing
            int statusID = 1;
            if (data.has("statusID") && !data.get("statusID").isJsonNull()) {
                statusID = data.get("statusID").getAsInt();
            }
            ...
            try (PreparedStatement stmt = conn.prepareStatement(insertOXP, Statement.RETURN_GENERATED_KEYS)) {
                ...
                stmt.setInt(4, statusID); // statusID from request
                stmt.setInt(5, currentUserId); // lastupdateuser_id

                int rowsAffected = stmt.executeUpdate();
                //system.out.println("object_x_people insert - rows affected: " + rowsAffected);

                if (rowsAffected == 0) {
                    throw new SQLException("Failed to insert object_x_people - no rows affected");
                }

                try (ResultSet generatedKeys = stmt.getGeneratedKeys()) {
                    if (generatedKeys.next()) {
                        objectXPeopleId = generatedKeys.getInt(1);
                        //system.out.println("Generated objectXPeopleId: " + objectXPeopleId);
                    } else {
            ...
            // Insert into system_x_objectxpeople
            String insertSXOP = """
                INSERT INTO system_x_objectxpeople (Object_x_ipid, SystemID, Last_UpdateUser_ID)
                VALUES (?, ?, ?)
                """;

            try (PreparedStatement stmt = conn.prepareStatement(insertSXOP)) {
                stmt.setInt(1, objectXPeopleId);
                stmt.setInt(2, systemId);
                stmt.setInt(3, currentUserId);

                int rowsAffected = stmt.executeUpdate();
                //system.out.println("system_x_objectxpeople insert - rows affected: " + rowsAffected);

                if (rowsAffected == 0) {
                    throw new SQLException("Failed to insert system_x_objectxpeople - no rows affected");
                }
            }
            ...
            insertStakeholderAudit(conn, systemId, "Name", null, fullName, "link", "Added", userName);
            insertStakeholderAudit(conn, systemId, "Role", null, roleName, "link", "Added", userName);
            insertStakeholderAudit(conn, systemId, "Role Status", null, statusName, "link", "Added", userName);
            ...
            com.example.budg_v2.util.RoleNotificationHelper.createNotificationAfterStakeholderAdded(
                "System", systemId, data.get("ipid").getAsInt(), 
            ...
            System.err.println("Error adding stakeholder: " + e.getMessage());
            conn.rollback();
            throw e;
        } finally {
    ...
    /**
     * Initialize stakeholders in a cloned system by copying all stakeholders from the original system.
     * This mirrors Glossary behavior: on first edit/view in changes, copy current stakeholders into clone.
     */
    @SuppressWarnings("unused")
    private void initializeStakeholdersInClonedSystem(Connection conn, int originalSystemId, int clonedSystemId, int currentUserId) throws SQLException {
        // Copy system_x_objectxpeople rows; object_x_people rows are shared by id, so we only link them.
        String sql = """
            INSERT INTO system_x_objectxpeople (Object_x_ipid, SystemID, Last_UpdateUser_ID)
            SELECT sxop.Object_x_ipid, ?, ?
            FROM system_x_objectxpeople sxop
            WHERE sxop.SystemID = ?
            """;

        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, clonedSystemId);
            stmt.setInt(2, currentUserId);
            stmt.setInt(3, originalSystemId);
            stmt.executeUpdate();
        }
    }
    ...
    private boolean updateStakeholder(Connection conn, JsonObject data, int currentUserId) throws SQLException {
        ...
        int systemId = getSystemIdForStakeholder(conn, objectXPeopleId);
        String userName = "System";
        ...
        if (changed && oldData != null && newData != null) {
            if (!safeEquals(oldData.fullName, newData.fullName))
                insertStakeholderAudit(conn, systemId, "Name", oldData.fullName, newData.fullName, "edit", "Updated", userName);
            if (!safeEquals(oldData.roleName, newData.roleName))
                insertStakeholderAudit(conn, systemId, "Role", oldData.roleName, newData.roleName, "edit", "Updated", userName);
            if (!safeEquals(oldData.statusName, newData.statusName))
                insertStakeholderAudit(conn, systemId, "Role Status", oldData.statusName, newData.statusName, "edit", "Updated", userName);
        }
        return true;
    }
    ...
    private boolean deleteStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        ...
        Integer facetTypeId = facetChangesDAO.getFacetId("system");
        if (facetTypeId != null && facetChangesDAO.hasActiveCRsBlockingStakeholderDeletion(facetTypeId, systemId)) {
        ...
        try {
            // Delete from system_x_objectxpeople first (foreign key constraint)
            String deleteSXOP = "DELETE FROM system_x_objectxpeople WHERE Object_x_ipid = ?";
            try (PreparedStatement stmt = conn.prepareStatement(deleteSXOP)) {
                stmt.setInt(1, objectXPeopleId);
                stmt.executeUpdate();
            }
            ...
            try (PreparedStatement stmt = conn.prepareStatement(deleteOXP)) {
                ...
                if (rowsAffected == 0) {
                    throw new SQLException("No stakeholder found with objectXPeopleId: " + objectXPeopleId);
                }
            }

            // Insert audit records for deletion
            if (oldData != null) {
                insertStakeholderAudit(conn, systemId, "Name", oldData.fullName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, systemId, "Role", oldData.roleName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, systemId, "Role Status", oldData.statusName, null, "delete", "Deleted", userName);
            }
            
            conn.commit();
            //system.out.println("Stakeholder deleted successfully");
            return true;

        } catch (SQLException e) {
            System.err.println("Error deleting stakeholder: " + e.getMessage());
            conn.rollback();
            throw e;
        } finally {
            conn.setAutoCommit(true);
    ...
    // Accept stakeholder status (change AcceptedID from 2 to 1)
    private boolean acceptStakeholderStatus(Connection conn, int objectXPeopleId, int currentUserId) throws SQLException {
        ...
        // Get system ID and old data for audit trail
        int systemId = getSystemIdForStakeholder(conn, objectXPeopleId);
        StakeholderSnapshot oldData = getStakeholderInfo(conn, objectXPeopleId);
        String userName = "System";
        try { userName = getCurrentUserNameAttribute(conn); } catch (Exception ignore) {}
        ...
        try {
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, systemId, "Role Status", oldData.statusName, "Yes", "status", "Accepted", userName);
            }
    ...
    // Extract system ID from request URI
    private int extractSystemId(String requestURI) {
        String[] uriParts = requestURI.split("/");
        for (int i = 0; i < uriParts.length; i++) {
            if ("system-stakeholder".equals(uriParts[i]) && i + 1 < uriParts.length) {
                try {
    ...
    private int getSystemIdForStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        String sql = "SELECT SystemID FROM system_x_objectxpeople WHERE Object_x_ipid = ? ORDER BY id DESC LIMIT 1";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, objectXPeopleId);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) return rs.getInt("SystemID");
            }
        }
        return -1;
    }
    ...
    /**
     * Build reference string in the format "FacetName ObjectId" (e.g., "System 10")
     */
    @SuppressWarnings("unused")
    ...
    private void insertStakeholderAudit(Connection conn, int systemId, String field, String oldValue, String newValue, String event, String updateType, String userName) throws SQLException {
        String mappedField = mapStakeholderField(field);
        String sql = "INSERT INTO system_audit_history (id, object, event, updateType, field, `from`, `to`, author) VALUES (?, 'Stakeholders', ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, systemId);

...
/**
 * businessarea Stakeholder Servlet for handling stakeholder CRUD operations
 * Endpoints: /api/businessarea/{id}/stakeholders, /api/businessarea/stakeholder/lookup
 */
@WebServlet({"/api/businessarea-stakeholder/*", "/api/businessarea/stakeholder/lookup"})
public class BusinessareaStakeholderServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(BusinessareaStakeholderServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            //system.out.println("businessareaStakeholderServlet - Request URI: " + requestURI);
            //system.out.println("businessareaStakeholderServlet - Path Info: " + pathInfo);
            ...
            if (pathInfo != null && (pathInfo.matches("/\\d+/stakeholders(/edit)?") || pathInfo.equals("/1/stakeholders") || pathInfo.equals("/1/stakeholders/edit"))) {
                ...
                int businessareaId = extractbusinessareaId(requestURI);
                //system.out.println("Extracted businessarea ID: " + businessareaId);

                if (businessareaId > 0) {
                    List<JsonObject> stakeholders = getbusinessareaStakeholders(conn, businessareaId);
                    //system.out.println("Found " + stakeholders.size() + " stakeholders");
                    out.print(gson.toJson(stakeholders));
                } else {
                    //system.out.println("Could not extract businessarea ID from URI: " + requestURI);
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    out.print("{\"error\": \"Invalid businessarea ID\"}");
                }
            } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                int businessareaId = extractbusinessareaId(request.getRequestURI());
                ...
                // Check Edit permission + stakeholder status for Business Area
                ...
                DefaultStakeholderUtil.ValidationResult canSave = DefaultStakeholderUtil.validateCurrentUserCanSaveStakeholders(
                        conn, "Businessarea", businessareaId, currentUserId, java.util.Collections.emptySet());
                if (!canSave.isValid()) {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                ...
                if (jsonObject.has("ipid")) {
                    ...
                    SegmentValidationService.ValidationResult segmentValidation =
                        segmentValidationService.validateStakeholderCanBeAdded(personId, businessareaId, "Business Area");
                ...
                // Add stakeholder
                boolean success = addStakeholder(conn, businessareaId, jsonObject, currentUserId);
    ...
    protected void doPatch(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/accept")) {
                int businessareaId = extractbusinessareaId(requestURI);
                //system.out.println("PATCH - Extracted businessarea ID: " + businessareaId);
    ...
    private void handleLookupRequest(HttpServletRequest request, HttpServletResponse response,
        ...
        switch (type.toLowerCase()) {
            case "roles":
                results = getbusinessareaRoles(conn);
                break;
            case "rolestatus":
    ...
    // Get businessarea stakeholders (works for both VIEW and EDIT modes)
    private List<JsonObject> getbusinessareaStakeholders(Connection conn, int businessareaId) throws SQLException {
        String sql = """
            SELECT 
            ...
            FROM businessarea_x_objectxpeople cxop
    ...
    // Get businessarea roles
    private List<JsonObject> getbusinessareaRoles(Connection conn) throws SQLException {
        String sql = """
            SELECT 
                orl.id AS RoleID,
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, getModuleId(conn, "businessarea"));
    ...
    @SuppressWarnings("unused") // Reserved for segment-filtered stakeholder selection
    private List<JsonObject> getPeopleByRoleWithSegmentFilter(Connection conn, int roleId, int objectId) throws SQLException {
        List<JsonObject> people = new ArrayList<>();
        
        try {
            List<Map<String, Object>> users = SegmentAccessService.getUsersForStakeholderSelection(objectId, "BusinessArea", roleId);
    ...
    // Add stakeholder
    private boolean addStakeholder(Connection conn, int businessareaId, JsonObject data, int currentUserId) throws SQLException {
        ...
        try {
            ...
            // Insert into businessarea_x_objectxpeople
            String insertCXOP = """
            ...
            try (PreparedStatement stmt = conn.prepareStatement(insertCXOP)) {
                ...
                int rowsAffected = stmt.executeUpdate();
                if (rowsAffected == 0) {
                    throw new SQLException("Failed to insert businessarea_x_objectxpeople");
                }
            }
            ...
            insertStakeholderAudit(conn, businessareaId, "Name", null, fullName, "link", "Added", userName);
            insertStakeholderAudit(conn, businessareaId, "Role", null, roleName, "link", "Added", userName);
            insertStakeholderAudit(conn, businessareaId, "Role Status", null, statusName, "link", "Added", userName);
            ...
            com.example.budg_v2.util.RoleNotificationHelper.createNotificationAfterStakeholderAdded(
                "Business Area", businessareaId, data.get("ipid").getAsInt(), 
    ...
    private boolean updateStakeholder(Connection conn, JsonObject data, int currentUserId) throws SQLException {
        ...
        int businessareaId = getBusinessAreaIdForStakeholder(conn, objectXPeopleId);
        String userName = "System";
        ...
        if (changed && oldData != null && newData != null) {
            if (!safeEquals(oldData.fullName, newData.fullName))
                insertStakeholderAudit(conn, businessareaId, "Name", oldData.fullName, newData.fullName, "edit", "Updated", userName);
            if (!safeEquals(oldData.roleName, newData.roleName))
                insertStakeholderAudit(conn, businessareaId, "Role", oldData.roleName, newData.roleName, "edit", "Updated", userName);
            if (!safeEquals(oldData.statusName, newData.statusName))
                insertStakeholderAudit(conn, businessareaId, "Role Status", oldData.statusName, newData.statusName, "edit", "Updated", userName);
        }
        return true;
    }
    ...
    private boolean deleteStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        ...
        Integer facetTypeId = facetChangesDAO.getFacetId("business-area");
        if (facetTypeId != null && facetChangesDAO.hasActiveCRsBlockingStakeholderDeletion(facetTypeId, businessareaId)) {
        ...
        try {
            // Delete from businessarea_x_objectxpeople first (foreign key constraint)
            String deleteCXOP = "DELETE FROM businessarea_x_objectxpeople WHERE Object_x_ipid = ?";
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, businessareaId, "Name", oldData.fullName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, businessareaId, "Role", oldData.roleName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, businessareaId, "Role Status", oldData.statusName, null, "delete", "Deleted", userName);
            }
    ...
    // Extract businessarea ID from request URI
    private int extractbusinessareaId(String requestURI) {
        String[] uriParts = requestURI.split("/");
        for (int i = 0; i < uriParts.length; i++) {
            if ("businessarea-stakeholder".equals(uriParts[i]) && i + 1 < uriParts.length) {
                try {
    ...
    private int getBusinessAreaIdForStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        String sql = "SELECT BusinessAreaID FROM businessarea_x_objectxpeople WHERE Object_x_ipid = ? ORDER BY id DESC LIMIT 1";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, objectXPeopleId);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) return rs.getInt("BusinessAreaID");
            }
        }
        return -1;
    }
    ...
    /**
     * Build reference string in the format "FacetName ObjectId" (e.g., "Business Area 5")
     */
    @SuppressWarnings("unused") // Reserved for change request / reference display
    ...
    private void insertStakeholderAudit(Connection conn, int businessareaId, String field, String oldValue, String newValue, String event, String updateType, String userName) throws SQLException {
        String mappedField = mapStakeholderField(field);
        String sql = "INSERT INTO business_area_audit_history (id, object, event, updateType, field, `from`, `to`, author) VALUES (?, 'Stakeholders', ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, businessareaId);
    ...
    private boolean acceptStakeholderStatus(Connection conn, int objectXPeopleId, int currentUserId) throws SQLException {
        ...
        try {
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, businessareaId, "Role Status", oldData.statusName, "Yes", "status", "Accepted", userName);
            }

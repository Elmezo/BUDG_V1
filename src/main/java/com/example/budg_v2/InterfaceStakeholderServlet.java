...
/**
 * interface Stakeholder Servlet for handling stakeholder CRUD operations
 * Endpoints: /api/interface/{id}/stakeholders, /api/interface/stakeholder/lookup
 */
@WebServlet({"/api/interface-stakeholder/*", "/api/interface/stakeholder/lookup"})
public class InterfaceStakeholderServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(InterfaceStakeholderServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            //system.out.println("interfaceStakeholderServlet - Request URI: " + requestURI);
            //system.out.println("interfaceStakeholderServlet - Path Info: " + pathInfo);
            ...
            if (pathInfo != null && (pathInfo.matches("/\\d+/stakeholders(/edit)?") || pathInfo.equals("/1/stakeholders") || pathInfo.equals("/1/stakeholders/edit"))) {
                ...
                int interfaceId = extractinterfaceId(requestURI);
                //system.out.println("Extracted interface ID: " + interfaceId);

                if (interfaceId > 0) {
                    List<JsonObject> stakeholders = getinterfaceStakeholders(conn, interfaceId);
                    //system.out.println("Found " + stakeholders.size() + " stakeholders");
                    out.print(gson.toJson(stakeholders));
                } else {
                    //system.out.println("Could not extract interface ID from URI: " + requestURI);
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    out.print("{\"error\": \"Invalid interface ID\"}");
                }
            } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                int interfaceId = extractinterfaceId(request.getRequestURI());
                ...
                // Check Edit permission + stakeholder status for Interface
                ...
                DefaultStakeholderUtil.ValidationResult canSave = DefaultStakeholderUtil.validateCurrentUserCanSaveStakeholders(
                        conn, "Interface", interfaceId, currentUserId, java.util.Collections.emptySet());
                if (!canSave.isValid()) {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                ...
                if (jsonObject.has("ipid")) {
                    ...
                    SegmentValidationService.ValidationResult segmentValidation =
                        segmentValidationService.validateStakeholderCanBeAdded(personId, interfaceId, "SystemInterface");
                ...
                // Add stakeholder
                boolean success = addStakeholder(conn, interfaceId, jsonObject, currentUserId);
    ...
    protected void doPatch(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/accept")) {
                int interfaceId = extractinterfaceId(requestURI);
                //system.out.println("PATCH - Extracted interface ID: " + interfaceId);
    ...
    private void handleLookupRequest(HttpServletRequest request, HttpServletResponse response,
        ...
        switch (type.toLowerCase()) {
            case "roles":
                results = getinterfaceRoles(conn);
                break;
            case "rolestatus":
    ...
    // Get interface stakeholders (works for both VIEW and EDIT modes)
    private List<JsonObject> getinterfaceStakeholders(Connection conn, int interfaceId) throws SQLException {
        String sql = """
            SELECT 
            ...
            FROM interface_x_objectxpeople cxop
    ...
    // Get interface roles
    private List<JsonObject> getinterfaceRoles(Connection conn) throws SQLException {
        String sql = """
            SELECT 
                orl.id AS RoleID,
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, getModuleId(conn, "interface"));
    ...
    private List<JsonObject> getPeopleByRoleWithSegmentFilter(Connection conn, int roleId, int objectId) throws SQLException {
        ...
        try {
            List<Map<String, Object>> users = SegmentAccessService.getUsersForStakeholderSelection(objectId, "SystemInterface", roleId);
    ...
    // Add stakeholder
    private boolean addStakeholder(Connection conn, int interfaceId, JsonObject data, int currentUserId) throws SQLException {
        ...
        try {
            ...
            // Insert into interface_x_objectxpeople
            String insertCXOP = """
            ...
            try (PreparedStatement stmt = conn.prepareStatement(insertCXOP)) {
                ...
                int rowsAffected = stmt.executeUpdate();
                if (rowsAffected == 0) {
                    throw new SQLException("Failed to insert interface_x_objectxpeople");
                }
            }
            ...
            insertStakeholderAudit(conn, interfaceId, "Name", null, fullName, "link", "Added", userName);
            insertStakeholderAudit(conn, interfaceId, "Role", null, roleName, "link", "Added", userName);
            insertStakeholderAudit(conn, interfaceId, "Role Status", null, statusName, "link", "Added", userName);
            ...
            com.example.budg_v2.util.RoleNotificationHelper.createNotificationAfterStakeholderAdded(
                "Interface", interfaceId, data.get("ipid").getAsInt(), 
    ...
    private boolean updateStakeholder(Connection conn, JsonObject data, int currentUserId) throws SQLException {
        ...
        int interfaceId = getInterfaceIdForStakeholder(conn, objectXPeopleId);
        String userName = "System";
        ...
        if (changed && oldData != null && newData != null) {
            if (!safeEquals(oldData.fullName, newData.fullName))
                insertStakeholderAudit(conn, interfaceId, "Name", oldData.fullName, newData.fullName, "edit", "Updated", userName);
            if (!safeEquals(oldData.roleName, newData.roleName))
                insertStakeholderAudit(conn, interfaceId, "Role", oldData.roleName, newData.roleName, "edit", "Updated", userName);
            if (!safeEquals(oldData.statusName, newData.statusName))
                insertStakeholderAudit(conn, interfaceId, "Role Status", oldData.statusName, newData.statusName, "edit", "Updated", userName);
        }
        return true;
    }
    ...
    private boolean deleteStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        ...
        Integer facetTypeId = facetChangesDAO.getFacetId("system-interface");
        if (facetTypeId != null && facetChangesDAO.hasActiveCRsBlockingStakeholderDeletion(facetTypeId, interfaceId)) {
        ...
        try {
            // Delete from interface_x_objectxpeople first (foreign key constraint)
            String deleteCXOP = "DELETE FROM interface_x_objectxpeople WHERE Object_x_ipid = ?";
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, interfaceId, "Name", oldData.fullName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, interfaceId, "Role", oldData.roleName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, interfaceId, "Role Status", oldData.statusName, null, "delete", "Deleted", userName);
            }
    ...
    private int extractinterfaceId(String requestURI) {
        String[] uriParts = requestURI.split("/");
        for (int i = 0; i < uriParts.length; i++) {
            if ("interface-stakeholder".equals(uriParts[i]) && i + 1 < uriParts.length) {
                try {
    ...
    private int getInterfaceIdForStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        String sql = "SELECT InterfaceID FROM interface_x_objectxpeople WHERE Object_x_ipid = ? ORDER BY id DESC LIMIT 1";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, objectXPeopleId);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) return rs.getInt("InterfaceID");
            }
        }
        return -1;
    }
    ...
    /**
     * Build reference string in the format "FacetName ObjectId" (e.g., "Interface 5")
     */
    @SuppressWarnings("unused")
    ...
    private boolean acceptStakeholderStatus(Connection conn, int objectXPeopleId, int currentUserId) throws SQLException {
        ...
        try {
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, interfaceId, "Role Status", oldData.statusName, "Yes", "status", "Accepted", userName);
            }

...
public class SystemImpactDAO {
    
    /**
     * Get the next available ID for product relationship table
     */
    private int getNextProductSystemId(Connection conn) throws SQLException {
        String sql = "SELECT COALESCE(MAX(ID), 0) + 1 FROM product_x_system";
     ...
     */
    private int getNextClientSystemId(Connection conn) throws SQLException {
        String sql = "SELECT COALESCE(MAX(ID), 0) + 1 FROM client_x_system";
        try (PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                return rs.getInt(1);
     ...
     */
    private int getNextLegalSystemId(Connection conn) throws SQLException {
        String sql = "SELECT COALESCE(MAX(ID), 0) + 1 FROM system_x_legal";
        try (PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                return rs.getInt(1);
    ...
    /**
     * Get product relationships for a system with owner and legal entity information
     */
    public List<Map<String, Object>> getProductRelationshipsBySystemId(int systemId) throws SQLException {
        ...
        String sql = """
            SELECT 
                pxs.ID as id,
                pxs.System_ID as systemId,
                pxs.Product_ID as productId,
                pxs.Product_System_Relation_Type as productSystemRelationType,
                pxs.Legal_ID as legalId,
                pxs.Legal_Relation_Type as legalRelationType,
                p.primaryname as productName,
                p.refnumber as productRefNumber,
                prt.PrimaryName as productSystemRelationTypeName,
                l.ShortName as legalShortName,
                l.LongName as legalLongName,
                lrt.PrimaryName as legalRelationTypeName
            ...
            LEFT JOIN legal l ON pxs.Legal_ID = l.ID
            LEFT JOIN product_x_legal_relationtype lrt ON pxs.Legal_Relation_Type = lrt.ID
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    relationship.put("productSystemRelationType", rs.getObject("productSystemRelationType"));
                    relationship.put("legalId", rs.getObject("legalId"));
                    relationship.put("legalRelationType", rs.getObject("legalRelationType"));
                    relationship.put("productName", rs.getString("productName"));
                    relationship.put("productRefNumber", rs.getString("productRefNumber"));
                    relationship.put("productSystemRelationTypeName", rs.getString("productSystemRelationTypeName"));
                    relationship.put("legalShortName", rs.getString("legalShortName"));
                    relationship.put("legalLongName", rs.getString("legalLongName"));
                    relationship.put("legalRelationTypeName", rs.getString("legalRelationTypeName"));
                    ...
                    try {
                        ...
                        //system.out.println("SystemImpactDAO: Error getting product owners: " + e.getMessage());
    ...
    /**
     * Get all product-system relation types
     */
    public List<Map<String, Object>> getProductSystemRelationTypes() throws SQLException {
        ...
        String sql = """
            ...
            FROM product_x_system_relationtype
    ...
    /**
     * Get all product-legal entity relation types
     */
    public List<Map<String, Object>> getProductLegalRelationTypes() throws SQLException {
        ...
        String sql = """
            ...
            FROM product_x_legal_relationtype
    ...
    /**
     * Save product relationships (includes Legal Entity relationships)
     */
    public boolean saveProductRelationships(int systemId, List<Map<String, Object>> relationships, int userId) throws SQLException {
        ...
        try {
            ...
            Map<Integer, Integer> existingRelationshipsMap = getExistingRelationshipsMap(conn, systemId, "product");
            ...
            if (relationships != null && !relationships.isEmpty()) {
                for (Map<String, Object> rel : relationships) {
                    ...
                    Object relationTypeObj = rel.get("productSystemRelationType");
            ...
            for (Map.Entry<Integer, Integer> entry : existingRelationshipsMap.entrySet()) {
                ...
                if (newRelationType == null) {
                    // Entity was deleted
                    logAuditHistoryForDelete(conn, systemId, "product", entityId, oldRelationType, userId);
                } else if (newRelationType != oldRelationType) {
                    // Relation type was updated
                    logAuditHistoryForUpdate(conn, systemId, "product", entityId, oldRelationType, newRelationType, userId);
                }
            }
            ...
            for (Map.Entry<Integer, Integer> entry : newRelationshipsMap.entrySet()) {
                ...
                if (!existingRelationshipsMap.containsKey(entityId)) {
                    // New entity added
                    logAuditHistoryForInsert(conn, systemId, "product", entityId, newRelationType, userId);
                }
            }
            
            // Delete existing relationships
            String deleteSql = "DELETE FROM product_x_system WHERE System_ID = ?";
            ...
            if (relationships != null && !relationships.isEmpty()) {
                String insertSql = """
                    INSERT INTO product_x_system 
                    (ID, Product_ID, System_ID, Legal_ID, Product_System_Relation_Type, Legal_Relation_Type, 
                     CreateDatetime, LastUpdateDatetime, LastUpdated_UserID) 
                    VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW(), ?)
                """;
                
                int nextId = getNextProductSystemId(conn);
                ...
                try (PreparedStatement ps = conn.prepareStatement(insertSql)) {
                    for (Map<String, Object> relationship : relationships) {
                        ...
                        Integer productSystemRelationType = relationship.get("productSystemRelationType") != null ? 
                            (relationship.get("productSystemRelationType") instanceof Integer ? (Integer) relationship.get("productSystemRelationType") : 
                             Integer.parseInt(String.valueOf(relationship.get("productSystemRelationType")))) : null;
                        Integer legalId = relationship.get("legalId") != null && !relationship.get("legalId").toString().isEmpty() ? 
                        ...
                        Integer legalRelationType = relationship.get("legalRelationType") != null && !relationship.get("legalRelationType").toString().isEmpty() ? 
                            (relationship.get("legalRelationType") instanceof Integer ? (Integer) relationship.get("legalRelationType") : 
                             Integer.parseInt(String.valueOf(relationship.get("legalRelationType")))) : null;
                        
                        if (productId == null || productSystemRelationType == null) {
                            continue;
                        }
                        
                        // Check for duplicate (productId + productSystemRelationType combination)
                        String uniqueKey = productId + "_" + productSystemRelationType;
                        if (seenRelationships.contains(uniqueKey)) {
                            //system.out.println("SystemImpactDAO: Skipping duplicate product relationship - productId=" + productId + ", productSystemRelationType=" + productSystemRelationType);
                            continue;
                        }
                        ...
                        ps.setInt(5, productSystemRelationType);
                        if (legalRelationType != null && legalRelationType > 0) {
                            ps.setInt(6, legalRelationType);
                        } else {
    ...
    /**
     * Get client relationships for a system with owner information
     */
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    try {
                        ...
                        //system.out.println("SystemImpactDAO: Error getting client owners: " + e.getMessage());
    ...
    /**
     * Get all client-system relation types
     */
    public List<Map<String, Object>> getClientSystemRelationTypes() throws SQLException {
        ...
        String sql = """
            SELECT ID, PrimaryName, Description, Priority, ReverseName
            FROM client_x_system_relationtype
            WHERE DeletedDatetime IS NULL
            ORDER BY Priority, PrimaryName
        """;
    ...
    public boolean saveClientRelationships(int systemId, List<Map<String, Object>> relationships, int userId) throws SQLException {
        ...
        try {
            ...
            // Get existing relationships before deleting (to compare with new ones)
            Map<Integer, Integer> existingRelationshipsMap = getExistingRelationshipsMap(conn, systemId, "client");
            ...
            for (Map.Entry<Integer, Integer> entry : existingRelationshipsMap.entrySet()) {
                ...
                if (newRelationType == null) {
                    // Entity was deleted
                    logAuditHistoryForDelete(conn, systemId, "client", entityId, oldRelationType, userId);
                } else if (newRelationType != oldRelationType) {
                    // Relation type was updated
                    logAuditHistoryForUpdate(conn, systemId, "client", entityId, oldRelationType, newRelationType, userId);
                }
            }
            ...
            for (Map.Entry<Integer, Integer> entry : newRelationshipsMap.entrySet()) {
                ...
                if (!existingRelationshipsMap.containsKey(entityId)) {
                    // New entity added
                    logAuditHistoryForInsert(conn, systemId, "client", entityId, newRelationType, userId);
                }
            }
            
            // Delete existing relationships
            String deleteSql = "DELETE FROM client_x_system WHERE System_ID = ?";
            try (PreparedStatement ps = conn.prepareStatement(deleteSql)) {
                ps.setInt(1, systemId);
                ps.executeUpdate();
            }
            ...
            if (relationships != null && !relationships.isEmpty()) {
                String insertSql = """
                    INSERT INTO client_x_system 
                    (ID, System_ID, Client_ID, RelationType, CreateDatetime, LastUpdateDatetime, LastUpdate_UserID) 
                    VALUES (?, ?, ?, ?, NOW(), NOW(), ?)
                """;
                
                int nextId = getNextClientSystemId(conn);
                Set<String> seenRelationships = new HashSet<>();
                ...
                try (PreparedStatement ps = conn.prepareStatement(insertSql)) {
                    for (Map<String, Object> relationship : relationships) {
                        ...
                        if (seenRelationships.contains(uniqueKey)) {
                            //system.out.println("SystemImpactDAO: Skipping duplicate client relationship - clientId=" + clientId + ", relationType=" + relationType);
                            continue;
                        }
                        seenRelationships.add(uniqueKey);
    ...
    /**
     * Get legal relationships for a system with owner information
     */
    public List<Map<String, Object>> getLegalRelationshipsBySystemId(int systemId) throws SQLException {
        String sql = """
            SELECT 
                sxl.ID as id,
                sxl.System_ID as system_id,
                sxl.Legal_ID as legal_id,
                sxl.RelationType as relation_type,
                l.ShortName as legalShortName,
                l.LongName as legalLongName,
                rt.PrimaryName as relationTypeName
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    try {
                        ...
                        //system.out.println("SystemImpactDAO: Error getting legal owners: " + e.getMessage());
    ...
    public List<Map<String, Object>> getLegalRelationTypes() throws SQLException {
        ...
        String sql = """
            SELECT ID, PrimaryName, Description, Priority, ReverseName
            FROM system_x_legal_relationtype
            WHERE DeletedDatetime IS NULL
            ORDER BY Priority, PrimaryName
        """;
    ...
    public boolean saveLegalRelationships(int systemId, List<Map<String, Object>> relationships, int userId) throws SQLException {
        ...
        try {
            ...
            // Get existing relationships before deleting (to compare with new ones)
            Map<Integer, Integer> existingRelationshipsMap = getExistingRelationshipsMap(conn, systemId, "legal");
            ...
            for (Map.Entry<Integer, Integer> entry : existingRelationshipsMap.entrySet()) {
                ...
                if (newRelationType == null) {
                    // Entity was deleted
                    logAuditHistoryForDelete(conn, systemId, "legal", entityId, oldRelationType, userId);
                } else if (newRelationType != oldRelationType) {
                    // Relation type was updated
                    logAuditHistoryForUpdate(conn, systemId, "legal", entityId, oldRelationType, newRelationType, userId);
                }
            }
            ...
            for (Map.Entry<Integer, Integer> entry : newRelationshipsMap.entrySet()) {
                ...
                if (!existingRelationshipsMap.containsKey(entityId)) {
                    // New entity added
                    logAuditHistoryForInsert(conn, systemId, "legal", entityId, newRelationType, userId);
                }
            }
            
            // Delete existing relationships
            String deleteSql = "DELETE FROM system_x_legal WHERE System_ID = ?";
            try (PreparedStatement ps = conn.prepareStatement(deleteSql)) {
                ps.setInt(1, systemId);
                ps.executeUpdate();
            }
            ...
            if (relationships != null && !relationships.isEmpty()) {
                String insertSql = """
                    INSERT INTO system_x_legal 
                    (ID, System_ID, Legal_ID, RelationType, CreateDatetime, LastUpdateDatetime, LastUpdateUser_ID) 
                    VALUES (?, ?, ?, ?, NOW(), NOW(), ?)
                """;
                
                int nextId = getNextLegalSystemId(conn);
                Set<String> seenRelationships = new HashSet<>();
                ...
                try (PreparedStatement ps = conn.prepareStatement(insertSql)) {
                    for (Map<String, Object> relationship : relationships) {
                        ...
                        if (seenRelationships.contains(uniqueKey)) {
                            //system.out.println("SystemImpactDAO: Skipping duplicate legal relationship - legalId=" + legalId + ", relationType=" + relationType);
                            continue;
                        }
                        seenRelationships.add(uniqueKey);
    ...
    /**
     * Get system relationships for a product (reverse lookup)
     */
    public List<Map<String, Object>> getSystemRelationshipsByProductId(int productId) throws SQLException {
        String sql = """
            SELECT 
                ...
                prt.ReverseName as relationTypeReverseName
            FROM product_x_system pxs
            LEFT JOIN system s ON pxs.System_ID = s.id
            LEFT JOIN product_x_system_relationtype prt ON pxs.Product_System_Relation_Type = prt.ID
            WHERE pxs.Product_ID = ? AND (s.Deleted_datetime IS NULL OR s.Deleted_datetime = '1970-01-01 00:00:00')
            ORDER BY s.Name
        """;
    ...
    /**
     * Get system relationships for a client (reverse lookup)
     */
    public List<Map<String, Object>> getSystemRelationshipsByClientId(int clientId) throws SQLException {
        String sql = """
            ...
            LEFT JOIN client_x_system_relationtype rt ON cxs.RelationType = rt.ID
            WHERE cxs.Client_ID = ? AND (s.Deleted_datetime IS NULL OR s.Deleted_datetime = '1970-01-01 00:00:00')
            ORDER BY s.Name
        """;
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    try {
                        String ownerName = getSystemOwnersString(systemId);
                        relationship.put("systemOwnerName", ownerName);
                    } catch (SQLException e) {
                        //system.out.println("SystemImpactDAO: SQLException getting system owners for systemId " + systemId + ": " + e.getMessage());
                        e.printStackTrace();
                        relationship.put("systemOwnerName", "No owner");
                    } catch (Exception e) {
                        //system.out.println("SystemImpactDAO: Exception getting system owners for systemId " + systemId + ": " + e.getMessage());
                        e.printStackTrace();
                        relationship.put("systemOwnerName", "No owner");
                    }
    ...
    /**
     * Get system relationships for a legal entity (reverse lookup)
     */
    public List<Map<String, Object>> getSystemRelationshipsByLegalId(int legalId) throws SQLException {
        String sql = """
            SELECT 
                ...
                rt.ReverseName as relationTypeReverseName
            FROM system_x_legal sxl
            LEFT JOIN system s ON sxl.System_ID = s.id
            LEFT JOIN system_x_legal_relationtype rt ON sxl.RelationType = rt.ID
            WHERE sxl.Legal_ID = ? AND (s.Deleted_datetime IS NULL OR s.Deleted_datetime = '1970-01-01 00:00:00')
            ORDER BY s.Name
        """;
    ...
    /**
     * Get the next available audit ID for system_audit_history table
     */
    private int getNextAuditId(Connection conn) throws SQLException {
        String sql = "SELECT COALESCE(MAX(auditidpk), 0) + 1 FROM system_audit_history";
        try (PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                return rs.getInt(1);
     ...
     */
    private Map<Integer, Integer> getExistingRelationshipsMap(Connection conn, int systemId, String tableType) throws SQLException {
        ...
        switch (tableType.toLowerCase()) {
            case "product":
                sql = "SELECT Product_ID, Product_System_Relation_Type FROM product_x_system WHERE System_ID = ?";
                entityColumn = "Product_ID";
                break;
            case "client":
                sql = "SELECT Client_ID, RelationType FROM client_x_system WHERE System_ID = ?";
                entityColumn = "Client_ID";
                break;
            case "legal":
                sql = "SELECT Legal_ID, RelationType FROM system_x_legal WHERE System_ID = ?";
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    int relationType = tableType.toLowerCase().equals("product") ? 
                        rs.getInt("Product_System_Relation_Type") : rs.getInt("RelationType");
    ...
    private String getEntityName(String tableType, int entityId) {
        ...
        switch (tableType.toLowerCase()) {
            ...
            case "client":
                ...
                break;
            case "legal":
                sql = "SELECT ShortName FROM legal WHERE ID = ?";
    ...
    private String getRelationTypeName(String tableType, int relationTypeId) {
        ...
        switch (tableType.toLowerCase()) {
            case "product":
                tableName = "product_x_system_relationtype";
                break;
            case "client":
                tableName = "client_x_system_relationtype";
                break;
            case "legal":
                tableName = "system_x_legal_relationtype";
                break;
            default:
                System.err.println("Unknown table type: " + tableType);
                return null;
     ...
     */
    private void logAuditHistoryForInsert(Connection conn, int systemId, String tableType, 
        ...
        try {
            ...
            switch (tableType.toLowerCase()) {
                case "product":
                    objectName = "System X Product";
                    entityFieldName = "Product";
                    break;
                case "client":
                    objectName = "System X Client";
                    entityFieldName = "Client";
                    break;
                case "legal":
                    objectName = "System X Legal";
     ...
     */
    private void logAuditHistoryForDelete(Connection conn, int systemId, String tableType, 
                                          int entityId, int relationTypeId, int userId) {
        try {
            // Get entity name
            String entityName = getEntityName(tableType, entityId);
     ...
     */
    private void logAuditHistoryForUpdate(Connection conn, int systemId, String tableType, 

...
public class ProcessDataDAO {
    ...
    /**
     * Get datasets for a process from process_X_dataset table
     * Also includes datasets from:
     * - attributes linked via process_X_attribute (even if not in process_X_dataset)
     * - systems linked via process_x_system (datasets where MasterSource = linked system)
    ...
    public List<Map<String, Object>> getProcessDatasets(int processId) throws SQLException {
        // First, get datasets explicitly linked via process_X_dataset
        String sqlExplicit = """
            SELECT 
                pxd.id,
                d.ID as datasetId,
        ...
        // Second, get datasets from attributes linked via process_X_attribute (but not in process_X_dataset)
        String sqlFromAttributes = """
            ...
            WHERE pxa.processid = ?
              ...
              AND d.ID NOT IN (
                  ...
                  FROM process_x_dataset 
                  WHERE processid = pxa.processid AND datasetid IS NOT NULL
              )
        """;
        
        // Third, get datasets from systems linked via process_x_system (but not already included)
        String sqlFromSystems = """
            ...
            LEFT JOIN dataset d ON d.MasterSource = pxs.system_id
            LEFT JOIN glossary g ON d.glossary = g.ID
            LEFT JOIN system s ON d.MasterSource = s.ID
    ...
    /**
     * Get attributes for a process from process_X_attribute table
     * Also includes attributes from:
     * - datasets linked via process_X_dataset (even if not in process_X_attribute)
     * - datasets inherited from systems linked via process_x_system
     */
    public List<Map<String, Object>> getProcessAttributes(int processId) throws SQLException {
        // First, get attributes explicitly linked via process_X_attribute
        String sqlExplicit = """
            SELECT 
                pxa.id,
                a.ID as attributeId,
        ...
        // Second, get attributes from datasets linked via process_X_dataset (but not in process_X_attribute)
        String sqlFromDatasets = """
            ...
            WHERE pxd.processid = ?
              ...
              AND a.ID NOT IN (
                  ...
                  FROM process_x_attribute 
                  WHERE processid = pxd.processid AND attributeid IS NOT NULL
              )
        """;
        
        // Third, get attributes from datasets inherited via linked systems.
        // Exclude attributes already explicitly linked in process_x_attribute.
        String sqlFromSystems = """
            SELECT 
            ...
            WHERE pxs.process_id = ?
              ...
              AND a.ID NOT IN (
                  SELECT COALESCE(attributeid, 0)
                  FROM process_x_attribute
                  WHERE processid = ? AND attributeid IS NOT NULL
              )
        """;

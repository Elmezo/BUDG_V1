...
/**
 * Handler for Legal Entity Role bulk uploads
 * Manages role assignments to legal entities through legal_x_objectxpeople table
 */
public class LegalEntityRoleHandler implements RoleUploadHandler {
    
    private static final Logger logger = LoggerFactory.getLogger(LegalEntityRoleHandler.class);
    private static final String ENTITY_NAME = "Legal Entity";
    private static final String LINKING_TABLE = "legal_x_objectxpeople";
    ...
    public void insert(Connection conn, JsonObject rowData, int userId) throws SQLException {
        ...
        Integer legalId = BulkUploadUtil.getInteger(rowData, "Legal_ID");
        Integer personId = BulkUploadUtil.getInteger(rowData, "Person_ID");
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        
        if (legalId == null) {
            throw new IllegalArgumentException("Legal_ID is required but was not resolved");
        }
        if (personId == null) {
        ...
        logger.debug("Inserting role assignment: Legal={}, Person={}, Role={}", legalId, personId, roleId);
        
        try {
            // Use LegalDAO for consistency with existing bulk upload logic
            ...
            if (objectXPeopleId == null) {
                ...
                objectXPeopleId = legalDAO.createObjectXPeople(conn, stakeholderData, userId);
            ...
            RoleHandlerUtil.checkLinkNotExists(conn, LINKING_TABLE, "Legal_ID", legalId, "Object_X_IP", objectXPeopleId, ENTITY_NAME);
            
            // Step 4: Link to legal entity
            legalDAO.linkStakeholderToLegal(conn, legalId, objectXPeopleId, userId);
            logger.debug("Linked stakeholder to legal entity: Legal={}, ObjectXPeople={}", legalId, objectXPeopleId);
            ...
            try {
                ...
                legalDAO.createStakeholderAuditRecord(legalId, userName, userFullName, roleId);
                logger.debug("Created audit records for legal entity stakeholder");
            } catch (Exception auditEx) {
                logger.warn("Failed to create audit records (non-fatal): {}", auditEx.getMessage());
                // Don't fail the insert if audit fails
            }
            
            logger.info("Successfully assigned role {} to person {} for legal entity {}", roleId, personId, legalId);
    ...
    public void delete(Connection conn, JsonObject rowData, int userId) throws SQLException {
        ...
        logger.debug("Deleting role assignment: Legal={}, Person={}, Role={}", legalId, personId, roleId);
        ...
        String deleteSql = "DELETE FROM " + LINKING_TABLE + " WHERE Legal_ID = ? AND Object_X_IP = ?";
        ...
        logger.info("Successfully removed {} role assignment(s) from legal entity: Legal={}, Person={}, Role={}",
            totalRowsAffected, legalId, personId, roleId);
    }
    
    @Override
    public JsonObject validateRow(JsonObject rowData, String operation, int userId) {
        // Validate basic IDs
        JsonObject basicValidation = RoleHandlerUtil.validateBasicIds(rowData, "Legal_ID", "Legal Entity");
        ...
        Integer personId = BulkUploadUtil.getInteger(rowData, "Person_ID");
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        if (legalId == null || personId == null || roleId == null) {
            return BulkUploadUtil.createValidationError("Legal_ID, Person_ID, and Role_ID are required.");
        }
        final int entityId = legalId.intValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (!SegmentAccessService.isSuperAdmin(userId)) {
                ...
                if (segmentObjectType != null && legalId != null) {
                    if (!SegmentAccessService.canAccessObject(userId, legalId, segmentObjectType)) {
            ...
            // Check if role belongs to Legal Entity module
            if (!RoleHandlerUtil.isRoleInModule(conn, roleIdVal, "Legal Entity")) {
                return BulkUploadUtil.createValidationError("Role is not assigned to Legal Entity module");
            }
            ...
            if ("INSERT".equalsIgnoreCase(operation) && personId != null && legalId != null) {
                try {
                    Long objectSegmentId = ObjectSegmentService.getObjectSegment((long) legalId, "Legal");
            ...
            if ("INSERT".equalsIgnoreCase(operation)) {
                if (RoleHandlerUtil.isDuplicateAssignment(conn, entityId, personIdVal, roleIdVal,
                        LINKING_TABLE, "Legal_ID", "Object_X_IP", ENTITY_NAME)) {
                    return BulkUploadUtil.createValidationError("This role assignment already exists for this Legal Entity");
                }

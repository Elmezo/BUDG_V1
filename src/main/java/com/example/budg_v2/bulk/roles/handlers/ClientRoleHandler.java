...
/**
 * Handler for Client Role bulk uploads
 * Manages role assignments to clients through client_x_objectxpeople table
 */
public class ClientRoleHandler implements RoleUploadHandler {
    
    private static final Logger logger = LoggerFactory.getLogger(ClientRoleHandler.class);
    private static final String ENTITY_NAME = "Client";
    private static final String LINKING_TABLE = "client_x_objectxpeople";
    ...
    public void insert(Connection conn, JsonObject rowData, int userId) throws SQLException {
        Integer clientId = BulkUploadUtil.getInteger(rowData, "Client_ID");
        Integer personId = BulkUploadUtil.getInteger(rowData, "Person_ID");
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        
        if (clientId == null || personId == null || roleId == null) {
            throw new IllegalArgumentException("Required IDs are missing");
        }
        
        logger.debug("Inserting role assignment: Client={}, Person={}, Role={}", clientId, personId, roleId);
        ...
        try {
            ...
            if (objectXPeopleId == null) {
                ...
                objectXPeopleId = clientDAO.createObjectXPeople(conn, stakeholderData, userId);
            ...
            RoleHandlerUtil.checkLinkNotExists(conn, LINKING_TABLE, "ClientID", clientId, "Object_x_ipid", objectXPeopleId, ENTITY_NAME);
            
            // Step 4: Link to client
            clientDAO.linkStakeholderToClient(conn, clientId, objectXPeopleId, userId);
            logger.debug("Linked stakeholder to client: Client={}, ObjectXPeople={}", clientId, objectXPeopleId);
            
            logger.info("Successfully assigned role {} to person {} for client {}", roleId, personId, clientId);
    ...
    public void delete(Connection conn, JsonObject rowData, int userId) throws SQLException {
        ...
        logger.debug("Deleting role assignment: Client={}, Person={}, Role={}", clientId, personId, roleId);
        ...
        String deleteSql = "DELETE FROM " + LINKING_TABLE + " WHERE ClientID = ? AND Object_x_ipid = ?";
        ...
        logger.info("Successfully removed {} role assignment(s) from client: Client={}, Person={}, Role={}",
            totalRowsAffected, clientId, personId, roleId);
    }
    
    @Override
    public JsonObject validateRow(JsonObject rowData, String operation, int userId) {
        JsonObject basicValidation = RoleHandlerUtil.validateBasicIds(rowData, "Client_ID", ENTITY_NAME);
        ...
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        if (clientId == null || personId == null || roleId == null) {
            return BulkUploadUtil.createValidationError("Client_ID, Person_ID, and Role_ID are required.");
        }
        final int entityId = clientId.intValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (!SegmentAccessService.isSuperAdmin(userId)) {
                ...
                if (segmentObjectType != null && clientId != null) {
                    if (!SegmentAccessService.canAccessObject(userId, clientId, segmentObjectType)) {
            ...
            if ("INSERT".equalsIgnoreCase(operation) && personId != null && clientId != null) {
                try {
                    Long objectSegmentId = ObjectSegmentService.getObjectSegment((long) clientId, ENTITY_NAME);
            ...
            if ("INSERT".equalsIgnoreCase(operation)) {
                if (RoleHandlerUtil.isDuplicateAssignment(conn, entityId, personIdVal, roleIdVal,
                        LINKING_TABLE, "ClientID", "Object_x_ipid", ENTITY_NAME)) {

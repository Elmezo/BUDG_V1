...
/**
 * Handler for Committee Role bulk uploads
 * Manages role assignments to committees through committee_x_objectxpeople table
 */
public class CommitteeRoleHandler implements RoleUploadHandler {
    
    private static final Logger logger = LoggerFactory.getLogger(CommitteeRoleHandler.class);
    private static final String ENTITY_NAME = "Committee";
    private static final String LINKING_TABLE = "committee_x_objectxpeople";
    ...
    public void insert(Connection conn, JsonObject rowData, int userId) throws SQLException {
        Integer committeeId = BulkUploadUtil.getInteger(rowData, "Committee_ID");
        Integer personId = BulkUploadUtil.getInteger(rowData, "Person_ID");
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        
        if (committeeId == null || personId == null || roleId == null) {
            throw new IllegalArgumentException("Required IDs are missing");
        }
        
        logger.debug("Inserting role assignment: Committee={}, Person={}, Role={}", committeeId, personId, roleId);
        ...
        try {
            ...
            if (objectXPeopleId == null) {
                ...
                objectXPeopleId = committeeDAO.createObjectXPeople(conn, stakeholderData, userId);
            ...
            RoleHandlerUtil.checkLinkNotExists(conn, LINKING_TABLE, "Committee_ID", committeeId, "Object_x_ipid", objectXPeopleId, ENTITY_NAME);
            
            // Step 4: Link to committee
            committeeDAO.linkStakeholderToCommittee(conn, committeeId, objectXPeopleId, userId);
            logger.debug("Linked stakeholder to committee: Committee={}, ObjectXPeople={}", committeeId, objectXPeopleId);
            
            logger.info("Successfully assigned role {} to person {} for committee {}", roleId, personId, committeeId);
    ...
    public void delete(Connection conn, JsonObject rowData, int userId) throws SQLException {
        ...
        logger.debug("Deleting role assignment: Committee={}, Person={}, Role={}", committeeId, personId, roleId);
        ...
        // Delete junction row for each object_x_people id linked to this committee
        int totalRowsAffected = 0;
        String deleteSql = "DELETE FROM " + LINKING_TABLE + " WHERE Committee_ID = ? AND Object_x_ipid = ?";
        ...
        logger.info("Successfully removed {} role assignment(s) from committee: Committee={}, Person={}, Role={}",
            totalRowsAffected, committeeId, personId, roleId);
    }
    
    @Override
    public JsonObject validateRow(JsonObject rowData, String operation, int userId) {
        JsonObject basicValidation = RoleHandlerUtil.validateBasicIds(rowData, "Committee_ID", ENTITY_NAME);
        ...
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        if (committeeId == null || personId == null || roleId == null) {
            return BulkUploadUtil.createValidationError("Committee_ID, Person_ID, and Role_ID are required.");
        }
        final int entityId = committeeId.intValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (!SegmentAccessService.isSuperAdmin(userId)) {
                ...
                if (segmentObjectType != null && committeeId != null) {
                    if (!SegmentAccessService.canAccessObject(userId, committeeId, segmentObjectType)) {
            ...
            if ("INSERT".equalsIgnoreCase(operation) && personId != null && committeeId != null) {
                try {
                    Long objectSegmentId = ObjectSegmentService.getObjectSegment((long) committeeId, ENTITY_NAME);
            ...
            if ("INSERT".equalsIgnoreCase(operation)) {
                if (RoleHandlerUtil.isDuplicateAssignment(conn, entityId, personIdVal, roleIdVal,
                        LINKING_TABLE, "Committee_ID", "Object_x_ipid", ENTITY_NAME)) {

...
/**
 * Handler for Capability Role bulk uploads
 * Manages role assignments to capabilities through capability_x_objectxpeople table
 */
public class CapabilityRoleHandler implements RoleUploadHandler {
    
    private static final Logger logger = LoggerFactory.getLogger(CapabilityRoleHandler.class);
    private static final String ENTITY_NAME = "Capability";
    private static final String LINKING_TABLE = "capability_x_objectxpeople";
    ...
    public void insert(Connection conn, JsonObject rowData, int userId) throws SQLException {
        Integer capabilityId = BulkUploadUtil.getInteger(rowData, "Capability_ID");
        Integer personId = BulkUploadUtil.getInteger(rowData, "Person_ID");
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        
        if (capabilityId == null || personId == null || roleId == null) {
            throw new IllegalArgumentException("Required IDs are missing");
        }
        
        logger.debug("Inserting role assignment: Capability={}, Person={}, Role={}", capabilityId, personId, roleId);
        ...
        try {
            ...
            if (objectXPeopleId == null) {
                ...
                objectXPeopleId = capabilityDAO.createObjectXPeople(conn, stakeholderData, userId);
            ...
            RoleHandlerUtil.checkLinkNotExists(conn, LINKING_TABLE, "CapabilityID", capabilityId, "Object_x_ipid", objectXPeopleId, ENTITY_NAME);
            
            // Step 4: Link to capability
            capabilityDAO.linkStakeholderToCapability(conn, capabilityId, objectXPeopleId, userId);
            logger.debug("Linked stakeholder to capability: Capability={}, ObjectXPeople={}", capabilityId, objectXPeopleId);
            
            logger.info("Successfully assigned role {} to person {} for capability {}", roleId, personId, capabilityId);
    ...
    public void delete(Connection conn, JsonObject rowData, int userId) throws SQLException {
        ...
        logger.debug("Deleting role assignment: Capability={}, Person={}, Role={}", capabilityId, personId, roleId);
        
        // Find ALL object_x_people IDs for this person+role combination (multiple can exist)
        ...
        // Delete junction row for each object_x_people id linked to this capability
        int totalRowsAffected = 0;
        String deleteSql = "DELETE FROM " + LINKING_TABLE + " WHERE CapabilityID = ? AND Object_x_ipid = ?";
        ...
        logger.info("Successfully removed {} role assignment(s) from capability: Capability={}, Person={}, Role={}",
            totalRowsAffected, capabilityId, personId, roleId);
    }
    
    @Override
    public JsonObject validateRow(JsonObject rowData, String operation, int userId) {
        JsonObject basicValidation = RoleHandlerUtil.validateBasicIds(rowData, "Capability_ID", ENTITY_NAME);
        ...
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        if (capabilityId == null || personId == null || roleId == null) {
            return BulkUploadUtil.createValidationError("Capability_ID, Person_ID, and Role_ID are required.");
        }
        final int entityId = capabilityId.intValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (!SegmentAccessService.isSuperAdmin(userId)) {
                ...
                if (segmentObjectType != null && capabilityId != null) {
                    if (!SegmentAccessService.canAccessObject(userId, capabilityId, segmentObjectType)) {
            ...
            if ("INSERT".equalsIgnoreCase(operation) && personId != null && capabilityId != null) {
                try {
                    Long objectSegmentId = ObjectSegmentService.getObjectSegment((long) capabilityId, ENTITY_NAME);
            ...
            if ("INSERT".equalsIgnoreCase(operation)) {
                if (RoleHandlerUtil.isDuplicateAssignment(conn, entityId, personIdVal, roleIdVal,
                        LINKING_TABLE, "CapabilityID", "Object_x_ipid", ENTITY_NAME)) {

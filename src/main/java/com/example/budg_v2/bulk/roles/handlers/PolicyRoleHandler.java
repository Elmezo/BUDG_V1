...
/**
 * Handler for Policy Role bulk uploads
 * Manages role assignments to policies through policy_x_objectxpeople table
 */
public class PolicyRoleHandler implements RoleUploadHandler {
    
    private static final Logger logger = LoggerFactory.getLogger(PolicyRoleHandler.class);
    private static final String ENTITY_NAME = "Policy";
    private static final String LINKING_TABLE = "policy_x_objectxpeople";
    ...
    public void insert(Connection conn, JsonObject rowData, int userId) throws SQLException {
        Integer policyId = BulkUploadUtil.getInteger(rowData, "Policy_ID");
        Integer personId = BulkUploadUtil.getInteger(rowData, "Person_ID");
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        
        if (policyId == null || personId == null || roleId == null) {
            throw new IllegalArgumentException("Required IDs are missing");
        }
        
        logger.debug("Inserting role assignment: Policy={}, Person={}, Role={}", policyId, personId, roleId);
        ...
        try {
            ...
            if (objectXPeopleId == null) {
                ...
                objectXPeopleId = policyDAO.createObjectXPeople(conn, stakeholderData, userId);
            ...
            RoleHandlerUtil.checkLinkNotExists(conn, LINKING_TABLE, "Policy_ID", policyId, "Object_X_IP", objectXPeopleId, ENTITY_NAME);
            
            // Step 4: Link to policy
            policyDAO.linkStakeholderToPolicy(conn, policyId, objectXPeopleId, userId);
            logger.debug("Linked stakeholder to policy: Policy={}, ObjectXPeople={}", policyId, objectXPeopleId);
            
            logger.info("Successfully assigned role {} to person {} for policy {}", roleId, personId, policyId);
    ...
    public void delete(Connection conn, JsonObject rowData, int userId) throws SQLException {
        ...
        logger.debug("Deleting role assignment: Policy={}, Person={}, Role={}", policyId, personId, roleId);
        ...
        String deleteSql = "DELETE FROM " + LINKING_TABLE + " WHERE Policy_ID = ? AND Object_X_IP = ?";
        ...
        logger.info("Successfully removed {} role assignment(s) from policy: Policy={}, Person={}, Role={}",
            totalRowsAffected, policyId, personId, roleId);
    }
    
    @Override
    public JsonObject validateRow(JsonObject rowData, String operation, int userId) {
        JsonObject basicValidation = RoleHandlerUtil.validateBasicIds(rowData, "Policy_ID", ENTITY_NAME);
        ...
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        if (policyId == null || personId == null || roleId == null) {
            return BulkUploadUtil.createValidationError("Policy_ID, Person_ID, and Role_ID are required.");
        }
        final int policyIdVal = policyId.intValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (!SegmentAccessService.isSuperAdmin(userId)) {
                ...
                if (segmentObjectType != null && policyId != null) {
                    if (!SegmentAccessService.canAccessObject(userId, policyId, segmentObjectType)) {
            ...
            if ("INSERT".equalsIgnoreCase(operation) && personId != null && policyId != null) {
                try {
                    Long objectSegmentId = ObjectSegmentService.getObjectSegment((long) policyId, ENTITY_NAME);
            ...
            if ("INSERT".equalsIgnoreCase(operation)) {
                if (RoleHandlerUtil.isDuplicateAssignment(conn, policyIdVal, personIdVal, roleIdVal,
                        LINKING_TABLE, "Policy_ID", "Object_X_IP", ENTITY_NAME)) {
                ...
                String errorMessage = RoleHandlerUtil.checkAssignmentExistsWithDetails(
                    conn, policyIdVal, personIdVal, roleIdVal,
                    LINKING_TABLE, "Policy_ID", "Object_X_IP", ENTITY_NAME,
                    "policy", "PrimaryName");

...
@WebServlet("/api/bulk/role/report/*")
public class RoleBulkReportServlet extends HttpServlet {

    private static final Logger logger = LoggerFactory.getLogger(RoleBulkReportServlet.class);
    ...
    private JsonArray getReportItems(Connection conn, int jobId) throws SQLException {
        // Schema: table `job_report_item` (lowercase), backticked columns for case-sensitive MySQL
        String itemsQuery = """
        ...
        try (PreparedStatement ps = conn.prepareStatement(itemsQuery)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    // created_at column removed as it doesn't exist in Job_Report_Item table

...
/**
 * Handler for Interface Role bulk uploads
 * Manages role assignments to interfaces through interface_x_objectxpeople table
 */
public class InterfaceRoleHandler implements RoleUploadHandler {
    
    private static final Logger logger = LoggerFactory.getLogger(InterfaceRoleHandler.class);
    private static final String ENTITY_NAME = "Interface";
    private static final String LINKING_TABLE = "interface_x_objectxpeople";
    ...
    public void insert(Connection conn, JsonObject rowData, int userId) throws SQLException {
        Integer interfaceId = BulkUploadUtil.getInteger(rowData, "Interface_ID");
        Integer personId = BulkUploadUtil.getInteger(rowData, "Person_ID");
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        
        if (interfaceId == null || personId == null || roleId == null) {
            throw new IllegalArgumentException("Required IDs are missing");
        }
        
        logger.debug("Inserting role assignment: Interface={}, Person={}, Role={}", interfaceId, personId, roleId);
        ...
        try {
            ...
            if (objectXPeopleId == null) {
                ...
                objectXPeopleId = interfaceDAO.createObjectXPeople(conn, stakeholderData, userId);
            ...
            RoleHandlerUtil.checkLinkNotExists(conn, LINKING_TABLE, "Interface_ID", interfaceId, "Object_x_ipid", objectXPeopleId, ENTITY_NAME);
            
            // Step 4: Link to interface
            interfaceDAO.linkStakeholderToInterface(conn, interfaceId, objectXPeopleId);
            logger.debug("Linked stakeholder to interface: Interface={}, ObjectXPeople={}", interfaceId, objectXPeopleId);
            
            logger.info("Successfully assigned role {} to person {} for interface {}", roleId, personId, interfaceId);
    ...
    public void delete(Connection conn, JsonObject rowData, int userId) throws SQLException {
        ...
        logger.debug("Deleting role assignment: Interface={}, Person={}, Role={}", interfaceId, personId, roleId);
        ...
        String deleteSql = "DELETE FROM " + LINKING_TABLE + " WHERE InterfaceID = ? AND Object_x_ipid = ?";
        ...
        logger.info("Successfully removed {} role assignment(s) from interface: Interface={}, Person={}, Role={}",
            totalRowsAffected, interfaceId, personId, roleId);
    }
    
    @Override
    public JsonObject validateRow(JsonObject rowData, String operation, int userId) {
        JsonObject basicValidation = RoleHandlerUtil.validateBasicIds(rowData, "Interface_ID", ENTITY_NAME);
        ...
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        if (interfaceId == null || personId == null || roleId == null) {
            return BulkUploadUtil.createValidationError("Interface_ID, Person_ID, and Role_ID are required.");
        }
        final int entityId = interfaceId.intValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (!SegmentAccessService.isSuperAdmin(userId)) {
                ...
                if (segmentObjectType != null && interfaceId != null) {
                    if (!SegmentAccessService.canAccessObject(userId, interfaceId, segmentObjectType)) {
            ...
            if ("INSERT".equalsIgnoreCase(operation) && personId != null && interfaceId != null) {
                try {
                    Long objectSegmentId = ObjectSegmentService.getObjectSegment((long) interfaceId, ENTITY_NAME);
            ...
            if ("INSERT".equalsIgnoreCase(operation)) {
                if (RoleHandlerUtil.isDuplicateAssignment(conn, entityId, personIdVal, roleIdVal,
                        LINKING_TABLE, "InterfaceID", "Object_x_ipid", ENTITY_NAME)) {

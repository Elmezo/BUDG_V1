...
/**
 * Handler for Process Role bulk uploads
 * Manages role assignments to processes through process_x_objectxpeople table
 */
public class ProcessRoleHandler implements RoleUploadHandler {
    
    private static final Logger logger = LoggerFactory.getLogger(ProcessRoleHandler.class);
    private static final String ENTITY_NAME = "Process";
    private static final String LINKING_TABLE = "process_x_objectxpeople";
    ...
    public void insert(Connection conn, JsonObject rowData, int userId) throws SQLException {
        Integer processId = BulkUploadUtil.getInteger(rowData, "Process_ID");
        Integer personId = BulkUploadUtil.getInteger(rowData, "Person_ID");
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        
        if (processId == null || personId == null || roleId == null) {
            throw new IllegalArgumentException("Required IDs are missing");
        }
        
        logger.debug("Inserting role assignment: Process={}, Person={}, Role={}", processId, personId, roleId);
        ...
        try {
            ...
            if (objectXPeopleId == null) {
                ...
                objectXPeopleId = processDAO.createObjectXPeople(conn, stakeholderData, userId);
            ...
            RoleHandlerUtil.checkLinkNotExists(conn, LINKING_TABLE, "process_id", processId, "object_x_ip", objectXPeopleId, ENTITY_NAME);
            
            // Step 4: Link to process
            processDAO.linkStakeholderToProcess(conn, processId, objectXPeopleId, userId);
            logger.debug("Linked stakeholder to process: Process={}, ObjectXPeople={}", processId, objectXPeopleId);
            
            logger.info("Successfully assigned role {} to person {} for process {}", roleId, personId, processId);
    ...
    public void delete(Connection conn, JsonObject rowData, int userId) throws SQLException {
        ...
        logger.debug("Deleting role assignment: Process={}, Person={}, Role={}", processId, personId, roleId);
        ...
        String deleteSql = "DELETE FROM " + LINKING_TABLE + " WHERE process_id = ? AND object_x_ip = ?";
        ...
        logger.info("Successfully removed {} role assignment(s) from process: Process={}, Person={}, Role={}",
            totalRowsAffected, processId, personId, roleId);
    }
    
    @Override
    public JsonObject validateRow(JsonObject rowData, String operation, int userId) {
        JsonObject basicValidation = RoleHandlerUtil.validateBasicIds(rowData, "Process_ID", ENTITY_NAME);
        ...
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        if (processId == null || personId == null || roleId == null) {
            return BulkUploadUtil.createValidationError("Process_ID, Person_ID, and Role_ID are required.");
        }
        final int processIdVal = processId.intValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (!SegmentAccessService.isSuperAdmin(userId)) {
                ...
                if (segmentObjectType != null && processId != null) {
                    if (!SegmentAccessService.canAccessObject(userId, processId, segmentObjectType)) {
            ...
            if ("INSERT".equalsIgnoreCase(operation) && personId != null && processId != null) {
                try {
                    Long objectSegmentId = ObjectSegmentService.getObjectSegment((long) processId, ENTITY_NAME);
            ...
            if ("INSERT".equalsIgnoreCase(operation)) {
                if (RoleHandlerUtil.isDuplicateAssignment(conn, processIdVal, personIdVal, roleIdVal,
                        LINKING_TABLE, "process_id", "object_x_ip", ENTITY_NAME)) {
                ...
                String errorMessage = RoleHandlerUtil.checkAssignmentExistsWithDetails(
                    conn, processIdVal, personIdVal, roleIdVal,
                    LINKING_TABLE, "process_id", "object_x_ip", ENTITY_NAME,
                    "process", "PrimaryName");
                ...
                try {
                    new BulkUploadCRValidationHelper().validateStakeholderRemovalAllowed("process", processIdVal);

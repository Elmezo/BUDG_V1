...
/**
 * Handler for Glossary Role bulk uploads
 * Manages role assignments to glossary items through glossary_x_objectxpeople table
 */
public class GlossaryRoleHandler implements RoleUploadHandler {
    
    private static final Logger logger = LoggerFactory.getLogger(GlossaryRoleHandler.class);
    private static final String ENTITY_NAME = "Glossary";
    private static final String LINKING_TABLE = "glossary_x_objectxpeople";
    ...
    public void insert(Connection conn, JsonObject rowData, int userId) throws SQLException {
        Integer glossaryId = BulkUploadUtil.getInteger(rowData, "Glossary_ID");
        Integer personId = BulkUploadUtil.getInteger(rowData, "Person_ID");
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        
        if (glossaryId == null || personId == null || roleId == null) {
            throw new IllegalArgumentException("Required IDs are missing");
        }
        
        logger.debug("Inserting role assignment: Glossary={}, Person={}, Role={}", glossaryId, personId, roleId);
        ...
        try {
            ...
            if (objectXPeopleId == null) {
                ...
                objectXPeopleId = glossaryDAO.createObjectXPeople(conn, stakeholderData, userId);
            ...
            RoleHandlerUtil.checkLinkNotExists(conn, LINKING_TABLE, "GlossaryID", glossaryId, "Object_x_ipid", objectXPeopleId, ENTITY_NAME);
            
            // Step 4: Link to glossary
            glossaryDAO.linkStakeholderToGlossary(conn, glossaryId, objectXPeopleId, userId);
            logger.debug("Linked stakeholder to glossary: Glossary={}, ObjectXPeople={}", glossaryId, objectXPeopleId);
            
            logger.info("Successfully assigned role {} to person {} for glossary {}", roleId, personId, glossaryId);
    ...
    public void delete(Connection conn, JsonObject rowData, int userId) throws SQLException {
        ...
        logger.debug("Deleting role assignment: Glossary={}, Person={}, Role={}", glossaryId, personId, roleId);
        ...
        String deleteSql = "DELETE FROM " + LINKING_TABLE + " WHERE GlossaryID = ? AND Object_x_ipid = ?";
        ...
        logger.info("Successfully removed {} role assignment(s) from glossary: Glossary={}, Person={}, Role={}",
            totalRowsAffected, glossaryId, personId, roleId);
    }
    
    @Override
    public JsonObject validateRow(JsonObject rowData, String operation, int userId) {
        JsonObject basicValidation = RoleHandlerUtil.validateBasicIds(rowData, "Glossary_ID", ENTITY_NAME);
        ...
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        if (glossaryId == null || personId == null || roleId == null) {
            return BulkUploadUtil.createValidationError("Glossary_ID, Person_ID, and Role_ID are required.");
        }
        final int entityId = glossaryId.intValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (!SegmentAccessService.isSuperAdmin(userId)) {
                ...
                if (segmentObjectType != null && glossaryId != null) {
                    if (!SegmentAccessService.canAccessObject(userId, glossaryId, segmentObjectType)) {
            ...
            if ("INSERT".equalsIgnoreCase(operation) && personId != null && glossaryId != null) {
                try {
                    Long objectSegmentId = ObjectSegmentService.getObjectSegment((long) glossaryId, ENTITY_NAME);
            ...
            if ("INSERT".equalsIgnoreCase(operation)) {
                if (RoleHandlerUtil.isDuplicateAssignment(conn, entityId, personIdVal, roleIdVal,
                        LINKING_TABLE, "GlossaryID", "Object_x_ipid", ENTITY_NAME)) {
                ...
                String errorMessage = RoleHandlerUtil.checkAssignmentExistsWithDetails(
                    ...
                    LINKING_TABLE, "GlossaryID", "Object_x_ipid", ENTITY_NAME,
                    "glossary", "PrimaryName");
                if (errorMessage != null) {
                    return BulkUploadUtil.createValidationError(errorMessage);
                }
                try {
                    new BulkUploadCRValidationHelper().validateStakeholderRemovalAllowed("glossary", entityId);

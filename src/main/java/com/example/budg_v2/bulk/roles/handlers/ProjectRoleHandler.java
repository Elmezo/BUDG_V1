...
/**
 * Handler for Project Role bulk uploads
 * Manages role assignments to projects through project_x_objectxpeople table
 * Note: Uses lowercase column naming (project_id, object_x_ip) unlike other handlers
 */
public class ProjectRoleHandler implements RoleUploadHandler {
    
    private static final Logger logger = LoggerFactory.getLogger(ProjectRoleHandler.class);
    private static final String ENTITY_NAME = "Project";
    private static final String LINKING_TABLE = "project_x_objectxpeople";
    ...
    public void insert(Connection conn, JsonObject rowData, int userId) throws SQLException {
        Integer projectId = BulkUploadUtil.getInteger(rowData, "Project_ID");
        Integer personId = BulkUploadUtil.getInteger(rowData, "Person_ID");
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        
        if (projectId == null || personId == null || roleId == null) {
            throw new IllegalArgumentException("Required IDs are missing");
        }
        
        logger.debug("Inserting role assignment: Project={}, Person={}, Role={}", projectId, personId, roleId);
        ...
        try {
            ...
            if (objectXPeopleId == null) {
                ...
                objectXPeopleId = projectDAO.createObjectXPeople(conn, stakeholderData, userId);
            ...
            RoleHandlerUtil.checkLinkNotExists(conn, LINKING_TABLE, "project_id", projectId, "object_x_ip", objectXPeopleId, ENTITY_NAME);
            
            // Step 4: Link to project
            projectDAO.linkStakeholderToProject(conn, projectId, objectXPeopleId);
            logger.debug("Linked stakeholder to project: Project={}, ObjectXPeople={}", projectId, objectXPeopleId);
            
            logger.info("Successfully assigned role {} to person {} for project {}", roleId, personId, projectId);
    ...
    public void delete(Connection conn, JsonObject rowData, int userId) throws SQLException {
        ...
        logger.debug("Deleting role assignment: Project={}, Person={}, Role={}", projectId, personId, roleId);
        ...
        String deleteSql = "DELETE FROM " + LINKING_TABLE + " WHERE project_id = ? AND object_x_ip = ?";
        ...
        logger.info("Successfully removed {} role assignment(s) from project: Project={}, Person={}, Role={}",
            totalRowsAffected, projectId, personId, roleId);
    }
    
    @Override
    public JsonObject validateRow(JsonObject rowData, String operation, int userId) {
        JsonObject basicValidation = RoleHandlerUtil.validateBasicIds(rowData, "Project_ID", ENTITY_NAME);
        ...
        Integer roleId = BulkUploadUtil.getInteger(rowData, "Role_ID");
        if (projectId == null || personId == null || roleId == null) {
            return BulkUploadUtil.createValidationError("Project_ID, Person_ID, and Role_ID are required.");
        }
        final int projectIdVal = projectId.intValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (!SegmentAccessService.isSuperAdmin(userId)) {
                ...
                if (segmentObjectType != null && projectId != null) {
                    if (!SegmentAccessService.canAccessObject(userId, projectId, segmentObjectType)) {
            ...
            if ("INSERT".equalsIgnoreCase(operation) && personId != null && projectId != null) {
                try {
                    Long objectSegmentId = ObjectSegmentService.getObjectSegment((long) projectId, ENTITY_NAME);
            ...
            // Special case: Project uses lowercase column names
            if ("INSERT".equalsIgnoreCase(operation)) {
                if (RoleHandlerUtil.isDuplicateAssignment(conn, projectIdVal, personIdVal, roleIdVal,
                        LINKING_TABLE, "project_id", "object_x_ip", ENTITY_NAME)) {
                ...
                String errorMessage = RoleHandlerUtil.checkAssignmentExistsWithDetails(
                    conn, projectIdVal, personIdVal, roleIdVal,
                    LINKING_TABLE, "project_id", "object_x_ip", ENTITY_NAME,
                    "project", "PrimaryName");

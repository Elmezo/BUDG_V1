...
/**
 * Servlet to provide job status for Capability bulk upload polling.
 ...
 */
@WebServlet("/api/bulk/capability/status/*")
public class CapabilityBulkStatusServlet extends HttpServlet {

    private static final Logger logger = LoggerFactory.getLogger(CapabilityBulkStatusServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logger.debug("Returned status for capability bulk job {}", jobId);
    ...
    private JsonObject getJobStatus(int jobId) throws SQLException {
        ...
        // Never show 0 failed when there are error report items (ensures GUI shows correct failed count)
        ...
        result.addProperty("is_completed", job.getCompletedDate() != null);
        result.addProperty("items_count", job.getItemsCount() != null ? job.getItemsCount() : 0);
    ...
    private JsonArray buildReportItemsJson(List<JobReportItem> reportItems) {
        ...
        for (JobReportItem item : reportItems) {
            ...
            o.addProperty("field_name", item.getFieldName());

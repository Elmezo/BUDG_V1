...
@WebServlet("/api/bulk/product/status/*")
public class ProductBulkStatusServlet extends HttpServlet {

    private static final Logger logger = LoggerFactory.getLogger(ProductBulkStatusServlet.class);
    ...
    private JsonObject getJobStatus(int jobId) throws SQLException {
        // Schema: `job` has ID, Status, Completed_date, Items_Count (no is_completed); `job_progress` has Job_ID, Status, Message (no progress_percentage, stage, updated_at)
        String query = """

...
@WebServlet("/api/bulk/interface/report/*")
public class InterfaceBulkReportServlet extends HttpServlet {

    private static final Logger logger = LoggerFactory.getLogger(InterfaceBulkReportServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            logger.error("Invalid Job ID format: {}", pathInfo, e);
            sendErrorResponse(response, "Invalid Job ID format. Please provide a valid numeric job ID.", 400);
        } catch (SQLException e) {
            logger.error("Database error while fetching report for job ID: {}", pathInfo != null ? pathInfo.substring(1) : "unknown", e);
            String errorMessage = e.getMessage() != null && !e.getMessage().isEmpty() 
                ? "Database error: " + e.getMessage() 
                : "Database error occurred while fetching the report. Please try again later.";
            sendErrorResponse(response, errorMessage, 500);
        } catch (Exception e) {
            logger.error("Unexpected error while fetching report for job ID: {}", pathInfo != null ? pathInfo.substring(1) : "unknown", e);
            String errorMessage = e.getMessage() != null && !e.getMessage().isEmpty() 
                ? "Internal server error: " + e.getMessage() 
                : "An unexpected error occurred while fetching the report. Please try again later.";
            sendErrorResponse(response, errorMessage, 500);
        }
    }

...
/**
 * Policy Stakeholder Servlet for handling stakeholder CRUD operations
 * Endpoints: /api/policy/{id}/stakeholders, /api/policy/stakeholder/lookup
 */
@WebServlet({"/api/policy-stakeholder/*", "/api/policy/stakeholder/lookup"})
public class PolicyStakeholderServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(PolicyStakeholderServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/view")) {
                int policyId = extractPolicyId(requestURI);

                if (policyId > 0) {
                    List<JsonObject> stakeholders = getPolicyStakeholdersForView(conn, policyId);
                    out.print(gson.toJson(stakeholders));
                } else {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    out.print("{\"error\": \"Invalid policy ID\"}");
                }
            }
            // Handle stakeholders edit requests  
                ...
                if (policyId > 0) {
                    List<JsonObject> stakeholders = getPolicyStakeholdersForEdit(conn, policyId);
                    out.print(gson.toJson(stakeholders));
                } else {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    out.print("{\"error\": \"Invalid policy ID\"}");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                int policyId = extractPolicyId(request.getRequestURI());
                ...
                // Check Edit permission + stakeholder status for Policy
                ...
                DefaultStakeholderUtil.ValidationResult canSave = DefaultStakeholderUtil.validateCurrentUserCanSaveStakeholders(
                        conn, "Policy", policyId, currentUserId, java.util.Collections.emptySet());
                if (!canSave.isValid()) {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                ...
                if (jsonObject.has("ipid")) {
                    ...
                    SegmentValidationService.ValidationResult segmentValidation =
                        segmentValidationService.validateStakeholderCanBeAdded(personId, policyId, "Policy");
                ...
                // Add stakeholder
                boolean success = addStakeholder(conn, policyId, jsonObject, currentUserId, request);
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                ...
                if (jsonObject.has("ipid")) {
                    ...
                    SegmentValidationService.ValidationResult segmentValidation =
                        segValSvc.validateStakeholderCanBeAdded(personId, policyId, "Policy");
                    if (!segmentValidation.isValid) {
                        response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                        out.print("{\"error\": " + gson.toJson(segmentValidation.message) + "}");
                        return;
    ...
    protected void doPatch(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/accept")) {
                int policyId = extractPolicyId(requestURI);
                //system.out.println("PATCH - Extracted policy ID: " + policyId);
    ...
    // Get policy stakeholders for VIEW mode (uses AcceptedID from roleaccepted)
    private List<JsonObject> getPolicyStakeholdersForView(Connection conn, int policyId) throws SQLException {
        String sql = """
            SELECT 
    ...
    // Get policy stakeholders for EDIT mode (uses statusID from object_x_ip_status)
    private List<JsonObject> getPolicyStakeholdersForEdit(Connection conn, int policyId) throws SQLException {
        String sql = """
            SELECT 
                oxp.id AS ObjectXPeopleID,
                oxp.isDelegateOF AS DelegateOfId,
    ...
    // Get policy roles
    private List<JsonObject> getPolicyRoles(Connection conn) throws SQLException {
        String sql = """
            SELECT 
                orl.id AS RoleID,
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, getModuleId(conn, "Policy"));
    ...
    private List<JsonObject> getPeopleByRoleWithSegmentFilter(Connection conn, int roleId, int objectId) throws SQLException {
        ...
        try {
            List<Map<String, Object>> users = SegmentAccessService.getUsersForStakeholderSelection(objectId, "Policy", roleId);
    ...
    // Add stakeholder
    private boolean addStakeholder(Connection conn, int policyId, JsonObject data, int currentUserId, HttpServletRequest request) throws SQLException {
        ...
        try {
            ...
            // Insert into policy_x_objectxpeople
            String insertPXOP = """
            ...
            try (PreparedStatement stmt = conn.prepareStatement(insertPXOP)) {
                ...
                int rowsAffected = stmt.executeUpdate();
                if (rowsAffected == 0) {
                    throw new SQLException("Failed to insert policy_x_objectxpeople");
                }
            }

            // =========== INSERT AUDIT ROW (يوضع قبل آخر قوس في السيرفلت) ===========
            String aUserName = getCurrentUserName(request);
            ...
            insertStakeholderAudit(conn, policyId, "Name", null, fullName, "link", "Added", aUserName);
            insertStakeholderAudit(conn, policyId, "Role", null, roleName, "link", "Added", aUserName);
            insertStakeholderAudit(conn, policyId, "Role Status", null, statusName, "link", "Added", aUserName);
            ...
            com.example.budg_v2.util.RoleNotificationHelper.createNotificationAfterStakeholderAdded(
                "Policy", policyId, data.get("ipid").getAsInt(), 
    ...
    private boolean updateStakeholder(Connection conn, JsonObject data, int currentUserId) throws SQLException {
        ...
        int policyId = getPolicyIdForStakeholder(conn, objectXPeopleId);
        ...
        String sql = "UPDATE object_x_people SET ipid=?, roleID=?, statusID=?, isDelegateOF=?, AcceptedID=2, lastupdatedatetime=NOW(), lastupdateuser_id=? WHERE id=?";
        ...
        if (changed && oldData != null && newData != null) {
            if (!safeEquals(oldData.fullName, newData.fullName))
                insertStakeholderAudit(conn, policyId, "Name", oldData.fullName, newData.fullName, "edit", "Updated", userName);
            if (!safeEquals(oldData.roleName, newData.roleName))
                insertStakeholderAudit(conn, policyId, "Role", oldData.roleName, newData.roleName, "edit", "Updated", userName);
            if (!safeEquals(oldData.statusName, newData.statusName))
                insertStakeholderAudit(conn, policyId, "Role Status", oldData.statusName, newData.statusName, "edit", "Updated", userName);
        }
        return true;
    }
    ...
    private boolean deleteStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        int policyId = getPolicyIdForStakeholder(conn, objectXPeopleId); // helper
        
        // Check if object has CRs that block stakeholder deletion
        // Auto CRs with "Pending Start" status allow stakeholder modification
        Integer facetTypeId = facetChangesDAO.getFacetId("policy");
        if (facetTypeId != null && facetChangesDAO.hasActiveCRsBlockingStakeholderDeletion(facetTypeId, policyId)) {
        ...
        conn.setAutoCommit(false);
        try {
            // Delete from policy_x_objectxpeople first (foreign key constraint)
            String deletePXOP = "DELETE FROM policy_x_objectxpeople WHERE object_x_ip = ?";
            ...
            try (PreparedStatement stmt = conn.prepareStatement(deleteOXP)) {
                stmt.setInt(1, objectXPeopleId);
                int rowsAffected = stmt.executeUpdate();
                if (rowsAffected == 0) throw new SQLException("No stakeholder found with the given ID");
            }
            // سجل audit الحذف
            if (oldData != null) {
                insertStakeholderAudit(conn, policyId, "Name", oldData.fullName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, policyId, "Role", oldData.roleName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, policyId, "Role Status", oldData.statusName, null, "delete", "Deleted", userName);
            }
            conn.commit();
            return true;
        } catch (SQLException e) {
    ...
    private boolean acceptStakeholderStatus(Connection conn, int objectXPeopleId, int currentUserId) throws SQLException {
        ...
        // Get policy ID and old data for audit trail
        int policyId = getPolicyIdForStakeholder(conn, objectXPeopleId);
        StakeholderSnapshot oldData = getStakeholderInfo(conn, objectXPeopleId);
        String userName = "System";
        try { userName = getCurrentUserNameAttribute(conn); } catch (Exception ignore) {}
        ...
        try {
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, policyId, "Role Status", oldData.statusName, "Yes", "status", "Accepted", userName);
            }
    ...
    // Extract policy ID from request URI
    private int extractPolicyId(String requestURI) {
        String[] uriParts = requestURI.split("/");
        for (int i = 0; i < uriParts.length; i++) {
            if ("policy-stakeholder".equals(uriParts[i]) && i + 1 < uriParts.length) {
                try {
    ...
    // معلومات الـ stakeholder من قاعدة البيانات
    ...
    // استرجاع policyId حسب objectXPeopleId
    private int getPolicyIdForStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        String sql = "SELECT policy_id FROM policy_x_objectxpeople WHERE object_x_ip = ? ORDER BY id DESC LIMIT 1";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, objectXPeopleId);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) return rs.getInt("policy_id");
            }
        }
        return -1;
    }
    ...
    /**
     * Build reference string in the format "FacetName ObjectId" (e.g., "Policy 5")
     */
    @SuppressWarnings("unused")
    ...
    private String getCurrentUserNameAttribute(Connection conn) { return "System"; /* مبدئياً */ }

    // في كلاس StakeholderSnapshot: احذف متغير objectXPeopleId لأنه غير مستخدم فعليًا
    private static class StakeholderSnapshot {
        String fullName; String roleName; String statusName;
    }

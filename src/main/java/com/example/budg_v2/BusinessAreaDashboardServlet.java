...
@WebServlet(name = "BusinessAreaDashboardServlet", urlPatterns = {"/api/business-area/dashboard/stats"})
public class BusinessAreaDashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try (Connection conn = com.example.budg_v2.database.DatabaseConnection.getConnection()) {
            ...
            // Parse filter parameters (excludeLifecycle, excludeStatus)
            Map<String, Set<String>> filters = parseFilters(req);
            
            // Build WHERE clause for filtering business_area records
            String filterWhereClause = buildFilterWhereClause(filters, conn);
            
            // Build segment filter condition
            String segmentFilter = buildSegmentFilter(userId, "business_area.id");
            
            // First, get total count of filtered non-deleted business_area records for verification
            String totalCountSql = "SELECT COUNT(*) as total FROM business_area WHERE deletedatetime IS NULL" + filterWhereClause + segmentFilter;
     ...
     * Supports: excludeLifecycle, excludeStatus
     */
    ...
    /**
     * Build WHERE clause to exclude filtered business_area records
     */
    private String buildFilterWhereClause(Map<String, Set<String>> filters, Connection conn) throws SQLException {
        ...
        if (filters.containsKey("lifecycle") && !filters.get("lifecycle").isEmpty()) {
            ...
            if (!lifecycleIds.isEmpty()) {
                ...
                conditions.add("business_area.Lifecycle NOT IN (" + idsStr + ")");
            }
        }
        ...
        if (filters.containsKey("status") && !filters.get("status").isEmpty()) {
            ...
            if (!statusIds.isEmpty()) {
                ...
                conditions.add("business_area.Status NOT IN (" + idsStr + ")");
            }
        }
    ...
    private List<Integer> getLifecycleIdsByName(Connection conn, Set<String> names) throws SQLException {
        ...
        String sql = "SELECT ID FROM business_area_lifecycle WHERE PrimaryName IN (" + placeholders + ")";
    ...
    /**
     * Build segment filter condition for business area
     */
    private String buildSegmentFilter(Integer userId, String idColumn) {
        if (userId != null && userId > 0) {
            try {
                String segmentCondition = SegmentAccessService.buildSelectedSegmentFilterClause(userId, "BusinessConnection", idColumn);
            ...
            return " AND (" +
                "EXISTS (" +
                    ...
                    "AND sot.Type = 'BusinessConnection' " +
    ...
    private List<Map<String, Object>> getLifecycleStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT bal.PrimaryName as label, COALESCE(COUNT(ba.ID), 0) as count " +
                     "FROM business_area_lifecycle bal " +
                     "LEFT JOIN (SELECT * FROM business_area WHERE deletedatetime IS NULL" + filterWhereClause + segmentFilter + ") ba ON ba.Lifecycle = bal.ID " +
                     "GROUP BY bal.ID, bal.PrimaryName " +
                     "ORDER BY bal.ID";
    ...
    private List<Map<String, Object>> getStatusStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT COALESCE(s.PrimaryName, s.primaryname) as label, COALESCE(COUNT(ba.ID), 0) as count " +
                     "FROM status s " +
                     "LEFT JOIN (SELECT * FROM business_area WHERE deletedatetime IS NULL" + filterWhereClause + segmentFilter + ") ba ON ba.Status = s.id " +

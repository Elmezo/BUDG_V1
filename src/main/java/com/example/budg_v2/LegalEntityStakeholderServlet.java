...
/**
 * Legal Entity Stakeholder Servlet for handling stakeholder CRUD operations
 * Endpoints: /api/legal-entity/{id}/stakeholders, /api/legal-entity/stakeholder/lookup
 */
@WebServlet({"/api/legal-entity-stakeholder/*", "/api/legal-entity/stakeholder/lookup"})
public class LegalEntityStakeholderServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(LegalEntityStakeholderServlet.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            //system.out.println("LegalEntityStakeholderServlet - Request URI: " + requestURI);
            //system.out.println("LegalEntityStakeholderServlet - Path Info: " + pathInfo);
            ...
            if (pathInfo != null && (pathInfo.matches("/\\d+/stakeholders(/edit)?") || pathInfo.equals("/1/stakeholders") || pathInfo.equals("/1/stakeholders/edit"))) {
                ...
                int legalEntityId = extractLegalEntityId(requestURI);
                //system.out.println("Extracted legal entity ID: " + legalEntityId);

                if (legalEntityId > 0) {
                    List<JsonObject> stakeholders = getLegalEntityStakeholders(conn, legalEntityId);
                    //system.out.println("Found " + stakeholders.size() + " stakeholders");
                    out.print(gson.toJson(stakeholders));
                } else {
                    //system.out.println("Could not extract legal entity ID from URI: " + requestURI);
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                    out.print("{\"error\": \"Invalid legal entity ID\"}");
                }
            } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/edit")) {
                int legalEntityId = extractLegalEntityId(request.getRequestURI());
                ...
                // Check Edit permission + stakeholder status for Legal Entity
                if (!PermissionCheckUtil.checkEditPermissionWithStakeholder(request, response, "Legal Entity", legalEntityId)) {
                ...
                DefaultStakeholderUtil.ValidationResult canSave = DefaultStakeholderUtil.validateCurrentUserCanSaveStakeholders(
                        conn, "LegalEntity", legalEntityId, currentUserId, java.util.Collections.emptySet());
                if (!canSave.isValid()) {
                    response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                ...
                if (jsonObject.has("ipid")) {
                    ...
                    SegmentValidationService.ValidationResult segmentValidation =
                        segmentValidationService.validateStakeholderCanBeAdded(personId, legalEntityId, "LegalEntity");
                ...
                // Add stakeholder
                boolean success = addStakeholder(conn, legalEntityId, jsonObject, currentUserId);
    ...
    protected void doPatch(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (pathInfo != null && pathInfo.matches("/\\d+/stakeholders/accept")) {
                int legalEntityId = extractLegalEntityId(requestURI);
                //system.out.println("PATCH - Extracted legal entity ID: " + legalEntityId);
    ...
    private void handleLookupRequest(HttpServletRequest request, HttpServletResponse response,
        ...
        switch (type.toLowerCase()) {
            case "roles":
                results = getLegalEntityRoles(conn);
                break;
            case "rolestatus":
    ...
    // Get legal entity stakeholders (works for both VIEW and EDIT modes)
    private List<JsonObject> getLegalEntityStakeholders(Connection conn, int legalEntityId) throws SQLException {
        String sql = """
            SELECT 
    ...
    private List<JsonObject> getLegalEntityRoles(Connection conn) throws SQLException {
        String sql = """
            SELECT 
                orl.id AS RoleID,
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, getModuleId(conn, "Legal Entity"));
    ...
    private List<JsonObject> getPeopleByRoleWithSegmentFilter(Connection conn, int roleId, int objectId) throws SQLException {
        ...
        try {
            List<Map<String, Object>> users = SegmentAccessService.getUsersForStakeholderSelection(objectId, "LegalEntity", roleId);
    ...
    // Add stakeholder
    private boolean addStakeholder(Connection conn, int legalEntityId, JsonObject data, int currentUserId) throws SQLException {
        ...
        try {
            ...
            // Insert into legal_x_objectxpeople
            String insertLXOP = """
                INSERT INTO legal_x_objectxpeople (Object_X_ip, Legal_ID, LastUpdate_UserID)
                VALUES (?, ?, ?)
                """;

            try (PreparedStatement stmt = conn.prepareStatement(insertLXOP)) {
                ...
                int rowsAffected = stmt.executeUpdate();
                if (rowsAffected == 0) {
                    throw new SQLException("Failed to insert legal_x_objectxpeople");
                }
            }
            ...
            insertStakeholderAudit(conn, legalEntityId, "Name", null, fullName, "link", "Added", userName);
            insertStakeholderAudit(conn, legalEntityId, "Role", null, roleName, "link", "Added", userName);
            insertStakeholderAudit(conn, legalEntityId, "Role Status", null, statusName, "link", "Added", userName);
            ...
            com.example.budg_v2.util.RoleNotificationHelper.createNotificationAfterStakeholderAdded(
                "Legal Entity", legalEntityId, data.get("ipid").getAsInt(), 
    ...
    private boolean updateStakeholder(Connection conn, JsonObject data, int currentUserId) throws SQLException {
        ...
        int legalEntityId = getLegalEntityIdForStakeholder(conn, objectXPeopleId);
        String userName = "System";
        ...
        if (changed && oldData != null && newData != null) {
            if (!safeEquals(oldData.fullName, newData.fullName))
                insertStakeholderAudit(conn, legalEntityId, "Name", oldData.fullName, newData.fullName, "edit", "Updated", userName);
            if (!safeEquals(oldData.roleName, newData.roleName))
                insertStakeholderAudit(conn, legalEntityId, "Role", oldData.roleName, newData.roleName, "edit", "Updated", userName);
            if (!safeEquals(oldData.statusName, newData.statusName))
                insertStakeholderAudit(conn, legalEntityId, "Role Status", oldData.statusName, newData.statusName, "edit", "Updated", userName);
        }
        return true;
    }
    ...
    private boolean deleteStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        ...
        Integer facetTypeId = facetChangesDAO.getFacetId("legal-entity");
        if (facetTypeId != null && facetChangesDAO.hasActiveCRsBlockingStakeholderDeletion(facetTypeId, legalEntityId)) {
        ...
        try {
            // Delete from legal_x_objectxpeople first (foreign key constraint)
            String deleteLXOP = "DELETE FROM legal_x_objectxpeople WHERE Object_X_ip = ?";
            try (PreparedStatement stmt = conn.prepareStatement(deleteLXOP)) {
                stmt.setInt(1, objectXPeopleId);
                stmt.executeUpdate();
            }
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, legalEntityId, "Name", oldData.fullName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, legalEntityId, "Role", oldData.roleName, null, "delete", "Deleted", userName);
                insertStakeholderAudit(conn, legalEntityId, "Role Status", oldData.statusName, null, "delete", "Deleted", userName);
            }
    ...
    // Extract legal entity ID from request URI
    private int extractLegalEntityId(String requestURI) {
        String[] uriParts = requestURI.split("/");
        for (int i = 0; i < uriParts.length; i++) {
            if ("legal-entity-stakeholder".equals(uriParts[i]) && i + 1 < uriParts.length) {
                try {
    ...
    private int getLegalEntityIdForStakeholder(Connection conn, int objectXPeopleId) throws SQLException {
        String sql = "SELECT Legal_ID FROM legal_x_objectxpeople WHERE Object_X_ip = ? ORDER BY id DESC LIMIT 1";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, objectXPeopleId);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) return rs.getInt("Legal_ID");
            }
        }
        return -1;
    }
    ...
    /**
     * Build reference string in the format "FacetName ObjectId" (e.g., "Legal Entity 5")
     */
    @SuppressWarnings("unused")
    ...
    private void insertStakeholderAudit(Connection conn, int legalEntityId, String field, String oldValue, String newValue, String event, String updateType, String userName) throws SQLException {
        String mappedField = mapStakeholderField(field);
        String sql = "INSERT INTO legal_audit_history (id, object, event, updateType, field, `from`, `to`, author) VALUES (?, 'Stakeholders', ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, legalEntityId);
    ...
    private boolean acceptStakeholderStatus(Connection conn, int objectXPeopleId, int currentUserId) throws SQLException {
        ...
        try {
            ...
            if (oldData != null) {
                insertStakeholderAudit(conn, legalEntityId, "Role Status", oldData.statusName, "Yes", "status", "Accepted", userName);
            }

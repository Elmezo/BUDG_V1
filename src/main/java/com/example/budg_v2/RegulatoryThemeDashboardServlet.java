...
@WebServlet(name = "RegulatoryThemeDashboardServlet", urlPatterns = {"/api/regulatory-theme/dashboard/stats"})
public class RegulatoryThemeDashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try (Connection conn = com.example.budg_v2.database.DatabaseConnection.getConnection()) {
            ...
            // Build WHERE clause for filtering regulatorytheme records
            String filterWhereClause = buildFilterWhereClause(filters, conn);
            
            // Build segment filter condition
            String segmentFilter = buildSegmentFilter(userId, "regulatorytheme.ID");
            
            // First, get total count of filtered non-deleted regulatorytheme records for verification
            String totalCountSql = "SELECT COUNT(*) as total FROM regulatorytheme WHERE DeletedDatetime IS NULL" + filterWhereClause + segmentFilter;
    ...
    /**
     * Build WHERE clause to exclude filtered regulatorytheme records
     */
    private String buildFilterWhereClause(Map<String, Set<String>> filters, Connection conn) throws SQLException {
        ...
        // Exclude by Status (using status table, no nulls for status)
        if (filters.containsKey("status") && !filters.get("status").isEmpty()) {
            ...
            if (!statusIds.isEmpty()) {
                ...
                conditions.add("regulatorytheme.Status_ID NOT IN (" + idsStr + ")");
            }
        }
    ...
    /**
     * Build segment filter condition for regulatory theme
     */
    private String buildSegmentFilter(Integer userId, String idColumn) {
        if (userId != null && userId > 0) {
            try {
                String segmentCondition = SegmentAccessService.buildSelectedSegmentFilterClause(userId, "RegulatoryTheme", idColumn);
    ...
    private List<Map<String, Object>> getStatusStats(Connection conn, String filterWhereClause, String segmentFilter) throws SQLException {
        ...
        String sql = "SELECT COALESCE(s.PrimaryName, s.primaryname) as label, COALESCE(COUNT(rt.ID), 0) as count " +
                     "FROM status s " +
                     "LEFT JOIN (SELECT * FROM regulatorytheme WHERE DeletedDatetime IS NULL" + filterWhereClause + segmentFilter + ") rt ON rt.Status_ID = s.id " +

...
@WebServlet(name = "InterfaceDataOutsideServlet", urlPatterns = {"/api/interface/data-outside"})
public class InterfaceDataOutsideServlet extends HttpServlet {

    private static final Logger LOGGER = Logger.getLogger(InterfaceDataOutsideServlet.class.getName());
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            List<Map<String, Object>> results = getDataOutsideInterface(interfaceId);
            ...
            LOGGER.log(Level.SEVERE, "SQL Error while fetching data outside interface", e);
            resp.getWriter().write(gson.toJson(new ArrayList<>()));
        } catch (Exception e) {
    ...
    private List<Map<String, Object>> getDataOutsideInterface(int interfaceId) throws SQLException {
        // First, get the source and target system IDs for this interface
        String interfaceSql = "SELECT Source_systemID, Target_systemID FROM interface WHERE id = ?";
        int sourceSystemId = 0;
        int targetSystemId = 0;
        
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(interfaceSql)) {
            ps.setInt(1, interfaceId);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    sourceSystemId = rs.getInt("Source_systemID");
                    targetSystemId = rs.getInt("Target_systemID");
                } else {
                    // Interface not found
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            LOGGER.log(Level.SEVERE, "SQL Exception in getDataOutsideInterface", e);
            throw e;
        }
